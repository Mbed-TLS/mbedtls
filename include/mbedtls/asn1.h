...
#define MBEDTLS_ASN1_IS_STRING_TAG(tag)                                                                            \
        ((tag) < 32u &&                                                                                            \
         (((1u << (tag)) &                                                                                         \
           ((1u << MBEDTLS_ASN1_BMP_STRING) | (1u << MBEDTLS_ASN1_UTF8_STRING) | (1u << MBEDTLS_ASN1_T61_STRING) | \
            (1u << MBEDTLS_ASN1_IA5_STRING) | (1u << MBEDTLS_ASN1_UNIVERSAL_STRING) |                              \
            (1u << MBEDTLS_ASN1_PRINTABLE_STRING) | (1u << MBEDTLS_ASN1_BIT_STRING))) != 0))
...
#define MBEDTLS_OID_CMP(oid_str, oid_buf) \
        ((MBEDTLS_OID_SIZE(oid_str) != (oid_buf)->len) || memcmp((oid_str), (oid_buf)->p, (oid_buf)->len) != 0)

#define MBEDTLS_OID_CMP_RAW(oid_str, oid_buf, oid_buf_len) \
        ((MBEDTLS_OID_SIZE(oid_str) != (oid_buf_len)) || memcmp((oid_str), (oid_buf), (oid_buf_len)) != 0)
 ...
 */
int mbedtls_asn1_traverse_sequence_of(unsigned char** p, const unsigned char* end, unsigned char tag_must_mask,
                                      unsigned char tag_must_val, unsigned char tag_may_mask, unsigned char tag_may_val,
                                      int (*cb)(void* ctx, int tag, unsigned char* start, size_t len), void* ctx);
 ...
 */
const mbedtls_asn1_named_data* mbedtls_asn1_find_named_data(const mbedtls_asn1_named_data* list, const char* oid,
                                                            size_t len);
