...
#define MBEDTLS_ERR_PK_PASSWORD_MISMATCH                               \
    -0x3B80 /**< Given private key password does not allow for correct \
               decryption. */
#define MBEDTLS_ERR_PK_INVALID_PUBKEY                                    \
    -0x3B00 /**< The pubkey tag or value is invalid (only RSA and EC are \
               supported). */
#define MBEDTLS_ERR_PK_INVALID_ALG \
    -0x3A80 /**< The algorithm tag or value is invalid. */
#define MBEDTLS_ERR_PK_UNKNOWN_NAMED_CURVE                           \
    -0x3A00 /**< Elliptic curve is unsupported (only NIST curves are \
               supported). */
...
#define MBEDTLS_ERR_PK_BUFFER_TOO_SMALL \
    -0x3880 /**< The output buffer is too small. */
