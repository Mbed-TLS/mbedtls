...
static inline int mbedtls_psa_get_ecc_oid_from_id(psa_ecc_family_t curve, size_t bits, char const** oid,
                                                  size_t* oid_len)
{
        switch (curve) {
...
static inline psa_key_type_t mbedtls_psa_parse_tls_ecc_group(uint16_t tls_ecc_grp_reg_id, size_t* bits)
{
        ...
        if (curve_info == NULL)
                return (0);
        return (PSA_KEY_TYPE_ECC_KEY_PAIR(mbedtls_ecc_group_to_psa(curve_info->MBEDTLS_PRIVATE(grp_id), bits)));
}
#endif /* MBEDTLS_ECP_C */
 ...
 * of making a copy.
 */
static inline int mbedtls_psa_tls_psa_ec_to_ecpoint(unsigned char* src, size_t srclen, unsigned char** dst,
                                                    size_t* dstlen)
{
        *dst = src;
        *dstlen = srclen;
 ...
 */
static inline int mbedtls_psa_tls_ecpoint_to_psa_ec(unsigned char const* src, size_t srclen, unsigned char* dst,
