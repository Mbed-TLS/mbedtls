...
#define MBEDTLS_ERR_SSL_NO_CLIENT_CERTIFICATE \
        -0x7480 /**< No client certification received from the client, but required by the authentication mode. */
#define MBEDTLS_ERR_SSL_UNSUPPORTED_EXTENSION \
        -0x7500 /**< Client received an extended server hello containing an unsupported extension */
...
#define MBEDTLS_ERR_SSL_PRIVATE_KEY_REQUIRED \
        -0x7600                                    /**< The own private key or pre-shared key is not set, but needed. */
...
#define MBEDTLS_ERR_SSL_PK_TYPE_MISMATCH \
        -0x6D00 /**< Public key type mismatch (eg, asked for RSA key exchange and presented EC key) */
...
#define MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER \
        -0x6600 /**< A field in a message was incorrect or inconsistent with other fields. */
#define MBEDTLS_ERR_SSL_CONTINUE_PROCESSING \
        -0x6580 /**< Internal-only message signaling that further message-processing should be done */
...
#define MBEDTLS_ERR_SSL_VERSION_MISMATCH \
        -0x5F00                            /**< An operation failed due to an unexpected version or configuration. */
 ...
 *
 */
typedef int mbedtls_ssl_cache_get_t(void* data, unsigned char const* session_id, size_t session_id_len,
 ...
 * \return                A non-zero return value on failure.
 */
typedef int mbedtls_ssl_cache_set_t(void* data, unsigned char const* session_id, size_t session_id_len,
 ...
 */
typedef int mbedtls_ssl_async_sign_t(mbedtls_ssl_context* ssl, mbedtls_x509_crt* cert, mbedtls_md_type_t md_alg,
 ...
 *                  directed in the documentation of this callback.
 */
typedef int mbedtls_ssl_async_decrypt_t(mbedtls_ssl_context* ssl, mbedtls_x509_crt* cert, const unsigned char* input,
                                        size_t input_len);
 ...
 *                  directed in the documentation of this callback.
 */
typedef int mbedtls_ssl_async_resume_t(mbedtls_ssl_context* ssl, unsigned char* output, size_t* output_len,
                                       size_t output_size);
 ...
 */
typedef void mbedtls_ssl_export_keys_t(void* p_expkey, mbedtls_ssl_key_export_type type, const unsigned char* secret,
...
struct mbedtls_ssl_config {
        ...
        int (*MBEDTLS_PRIVATE(f_ticket_write))(void*, const mbedtls_ssl_session*, unsigned char*, const unsigned char*,
                                               size_t*, uint32_t*);
        ...
        mbedtls_ssl_async_decrypt_t* MBEDTLS_PRIVATE(f_async_decrypt_start); /*!< start asynchronous decryption
                                                                                operation */
#endif                                                                       /* MBEDTLS_X509_CRT_PARSE_C */
 ...
 */
typedef int mbedtls_ssl_ticket_write_t(void* p_ticket, const mbedtls_ssl_session* session, unsigned char* start,
 ...
 */
typedef int mbedtls_ssl_cookie_write_t(void* ctx, unsigned char** p, unsigned char* end, const unsigned char* info,
                                       size_t ilen);
 ...
 *                 or a negative error code.
 */
typedef int mbedtls_ssl_cookie_check_t(void* ctx, const unsigned char* cookie, size_t clen, const unsigned char* info,
                                       size_t ilen);
 ...
 */
void mbedtls_ssl_conf_dtls_cookies(mbedtls_ssl_config* conf, mbedtls_ssl_cookie_write_t* f_cookie_write,
