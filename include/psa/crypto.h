 ...
 *   \code
 *   psa_sign_hash_interruptible_operation_t operation =
 *   PSA_SIGN_HASH_INTERRRUPTIBLE_OPERATION_INIT;
 *   \endcode
 ...
 *                              far.
 */
uint32_t psa_verify_get_num_ops(
                         psa_verify_hash_interruptible_operation_t *operation );
 ...
 * Aborting an operation frees all associated resources except for the
 * \p operation structure itself. Once aborted, the operation object
 * can be reused for another operation by calling \c psa_sign_hash_start()
 * again.
 *
 * You may call this function any time after the operation object has
 * been initialized.
 ...
 * Aborting an operation frees all associated resources except for the
 * \p operation structure itself. Once aborted, the operation object
 * can be reused for another operation by calling \c psa_verify_hash_start()
 * again.
 *
 * You may call this function any time after the operation object has
 * been initialized.
 *
 * In particular, calling \c psa_verify_hash_abort() after the operation has
 * already been terminated by a call to \c psa_verify_hash_abort() or
 * \c psa_verify_hash_complete() is safe and has no effect.
 *
