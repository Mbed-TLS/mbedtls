...
#define PSA_ALG_IS_DSA(alg) (((alg) & ~PSA_ALG_HASH_MASK & ~PSA_ALG_DSA_DETERMINISTIC_FLAG) == PSA_ALG_DSA_BASE)
 ...
 * \retval #PSA_ERROR_INSUFFICIENT_MEMORY
 */
psa_status_t psa_set_key_domain_parameters(psa_key_attributes_t* attributes, psa_key_type_t type, const uint8_t* data,
                                           size_t data_length);
 ...
 */
psa_status_t psa_get_key_domain_parameters(const psa_key_attributes_t* attributes, uint8_t* data, size_t data_size,
                                           size_t* data_length);
...
#define PSA_KEY_DOMAIN_PARAMETERS_SIZE(key_type, key_bits)     \
        (PSA_KEY_TYPE_IS_RSA(key_type) ?                       \
                 ...
                 PSA_KEY_TYPE_IS_DSA(key_type) ? PSA_DSA_KEY_DOMAIN_PARAMETERS_SIZE(key_bits) : 0)
...
{
        return ((key_id >= MBEDTLS_PSA_KEY_ID_BUILTIN_MIN) && (key_id <= MBEDTLS_PSA_KEY_ID_BUILTIN_MAX));
}
 ...
 */
psa_status_t mbedtls_psa_platform_get_builtin_key(mbedtls_svc_key_id_t key_id, psa_key_lifetime_t* lifetime,
...
#define PSA_PAKE_PRIMITIVE(pake_type, pake_family, pake_bits) \
        ((pake_bits & 0xFFFF) != pake_bits) ?                 \
                0 :                                           \
                ((psa_pake_primitive_t)(((pake_type) << 24 | (pake_family) << 16) | (pake_bits)))
 ...
 *         results in this error code.
 */
psa_status_t psa_pake_output(psa_pake_operation_t* operation, psa_pake_step_t step, uint8_t* output, size_t output_size,
...
{
        return (PSA_PAKE_PRIMITIVE(cipher_suite->type, cipher_suite->family, cipher_suite->bits));
}
