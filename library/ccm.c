...
static int ccm_calculate_first_block_if_ready(mbedtls_ccm_context *ctx)
{
     ...
     */
    if (ctx->tag_len == 0) {
        if (ctx->mode == MBEDTLS_CCM_STAR_ENCRYPT ||
    ...
    for (i = 0, len_left = ctx->plaintext_len; i < ctx->q;
         i++, len_left >>= 8) {
    ...
    if ((ret =
             mbedtls_cipher_update(&ctx->cipher_ctx, ctx->y, 16, ctx->y,
                                   &olen)) != 0) {
        ctx->state |= CCM_STATE__ERROR;
        return ret;
    }
    ...
    if (add_len > 0) {
        ...
        while (add_len > 0) {
            ...
            if (use_len + offset == 16 || ctx->processed == ctx->add_len) {
                if ((ret =
                         mbedtls_cipher_update(&ctx->cipher_ctx, ctx->y, 16,
                                               ctx->y,
                                               &olen)) != 0) {
                    ctx->state |= CCM_STATE__ERROR;
                    return ret;
    ...
    while (input_len > 0) {
        ...
        if (ctx->mode == MBEDTLS_CCM_ENCRYPT || \
            ...
            if (use_len + offset == 16 ||
                ctx->processed == ctx->plaintext_len) {
                if ((ret =
                         mbedtls_cipher_update(&ctx->cipher_ctx, ctx->y, 16,
                                               ctx->y,
                                               &olen)) != 0) {
...
static int mbedtls_ccm_compare_tags(const unsigned char *tag1,
                                    const unsigned char *tag2, size_t tag_len)
{
    unsigned char i;
    int diff;
