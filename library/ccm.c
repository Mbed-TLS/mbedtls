                                  ...
                                  unsigned char* output, const unsigned char* tag, size_t tag_len)
{
        ...
        if ((ret = ccm_auth_crypt(ctx, CCM_DECRYPT, length, iv, iv_len, add, add_len, input, output, check_tag,
                                  tag_len)) != 0) {
                return (ret);
        }
...
int mbedtls_ccm_self_test(int verbose)
{
        ...
        for (i = 0; i < NB_TESTS; i++) {
                ...
                ret = mbedtls_ccm_encrypt_and_tag(&ctx, msg_len_test_data[i], iv_test_data, iv_len_test_data[i],
                ...
                ret = mbedtls_ccm_auth_decrypt(&ctx, msg_len_test_data[i], iv_test_data, iv_len_test_data[i],
