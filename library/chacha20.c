...
static const unsigned char test_xnonces[24] =
{
...
static const unsigned char test_xinput[304] =
{
...
static const unsigned char test_xoutput[2][304] =
{
    {
...
{
    unsigned char output[304];
    unsigned i;
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    for (i = 0U; i < 2U; i++) {
        if (verbose != 0) {
            mbedtls_printf("  XChaCha20 test %u ", i);
        }

        ret = mbedtls_xchacha20_crypt(test_xkeys,
                                      test_xnonces,
                                      test_counters[i],
                                      304,
                                      test_xinput,
        ...
        ASSERT(0 == memcmp(output, test_xoutput[i], 304),
               ("failed (output)\n"));
