    ...
    *p++ = MBEDTLS_CTR_DRBG_SEEDLEN;
    ...
    for( j = 0; j < MBEDTLS_CTR_DRBG_SEEDLEN; j += MBEDTLS_CTR_DRBG_BLOCKSIZE )
    {
        ...
        while( use_len > 0 )
        {
            ...
            MBEDTLS_TRACE_OP_CTX( "block_cipher_df-1", &aes_ctx ); 
    ...
    for( j = 0; j < MBEDTLS_CTR_DRBG_SEEDLEN; j += MBEDTLS_CTR_DRBG_BLOCKSIZE )
    {
        MBEDTLS_TRACE_OP_CTX( "block_cipher_df-2", &aes_ctx ); 
        if( ( ret = mbedtls_aes_crypt_ecb( &aes_ctx, MBEDTLS_AES_ENCRYPT,
                                           iv, iv ) ) != 0 )
        {
            goto exit;
                          ...
                          const unsigned char data[MBEDTLS_CTR_DRBG_SEEDLEN] )
{
    ...
    for( j = 0; j < MBEDTLS_CTR_DRBG_SEEDLEN; j += MBEDTLS_CTR_DRBG_BLOCKSIZE )
    {
         ...
         * Crypt counter block
         */
        MBEDTLS_TRACE_OP_CTX_CTX( "ctr_drbg_update_internal", ctx,
                                 &ctx->aes_ctx ); 
                              ...
                              const unsigned char *additional, size_t add_len )
{
    ...
    while( output_len > 0 )
    {
         ...
         * Crypt counter block
         */
        MBEDTLS_TRACE_OP_CTX_CTX( "mbedtls_ctr_drbg_random_with_add", ctx,
                                 &ctx->aes_ctx );
        if( ( ret = mbedtls_aes_crypt_ecb( &ctx->aes_ctx, MBEDTLS_AES_ENCRYPT,
                                           ctx->counter, tmp ) ) != 0 )
        {
