...
#define D_CONST_LEN            (2)
static const unsigned char D_PUBLIC_CONSTANT_BYTES[D_CONST_LEN] =
{ 0x80, 0x80 };
static const unsigned char D_MESSAGE_CONSTANT_BYTES[D_CONST_LEN] =
{ 0x81, 0x81 };
 ...
 */
static unsigned short lmots_checksum_calculate(
    ...
    unsigned char *out)
{
    ...
    mbedtls_lms_unsigned_int_to_network_bytes(checksum, CHECKSUM_LEN,
                                              out +
                                              MBEDTLS_LMOTS_N_HASH_LEN(params->
                                                                       type));
 ...
 */
static int public_key_from_hashed_digit_array(
    const mbedtls_lmots_parameters_t *params,
    ...
    size_t *out_len)
{
    ...
    ret = create_digit_array_with_checksum(params,
                                           msg,
                                           msg_size,
    ...
    ret = hash_digit_array(params,
                           sig + MBEDTLS_LMOTS_SIG_SIGNATURE_OFFSET(
                               params->type),
                           tmp_digit_array,
                           NULL,
                       ...
                       unsigned char *sig, size_t sig_size, size_t *sig_len)
{
     ...
     * key.
     */
    unsigned char tmp_sig[MBEDTLS_LMOTS_P_SIG_DIGIT_COUNT_MAX][
        MBEDTLS_LMOTS_N_HASH_LEN_MAX];
