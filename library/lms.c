...
#define MERKLE_TREE_NODE_AM(type)          (1u << \
                                            (MBEDTLS_LMS_H_TREE_HEIGHT(type) + \
                                             1u))
#define MERKLE_TREE_LEAF_NODE_AM(type)     (1u << MBEDTLS_LMS_H_TREE_HEIGHT( \
                                                type))
#define MERKLE_TREE_INTERNAL_NODE_AM(type) (1u << MBEDTLS_LMS_H_TREE_HEIGHT( \
                                                type))
                       ...
                       const unsigned char *sig, size_t sig_size)
{
    ...
    if (sig_size <
        SIG_TYPE_OFFSET(ctx->params.otstype) + MBEDTLS_LMS_TYPE_LEN) {
        return MBEDTLS_ERR_LMS_VERIFY_FAILED;
    }

    if (mbedtls_lms_network_bytes_to_unsigned_int(MBEDTLS_LMS_TYPE_LEN,
                                                  sig +
                                                  SIG_TYPE_OFFSET(ctx->params.
                                                                  otstype))
        != MBEDTLS_LMS_SHA256_M32_H10) {
        return MBEDTLS_ERR_LMS_VERIFY_FAILED;
    }
...
void mbedtls_lms_private_free(mbedtls_lms_private_t *ctx)
{
    ...
    if (ctx->have_private_key) {
        if (ctx->ots_private_keys != NULL) {
            for (idx = 0;
                 idx < MERKLE_TREE_LEAF_NODE_AM(ctx->params.type);
                 idx++) {
                                     ...
                                     size_t seed_size)
{
    ...
    ctx->ots_private_keys =
        mbedtls_calloc((size_t)MERKLE_TREE_LEAF_NODE_AM(ctx->params.type),
                     ...
                     size_t *sig_len)
{
    ...
    ret = get_merkle_path(ctx,
                          MERKLE_TREE_INTERNAL_NODE_AM(
                              ctx->params.type) + q_leaf_identifier,
