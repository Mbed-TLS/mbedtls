...
int mbedtls_mps_reader_init(mbedtls_mps_reader* rd, unsigned char* acc, mbedtls_mps_size_t acc_len)
{
        MBEDTLS_MPS_TRACE_INIT("mbedtls_mps_reader_init");
        MBEDTLS_MPS_TRACE(MBEDTLS_MPS_TRACE_TYPE_COMMENT, "* Accumulator size: %u bytes", (unsigned)acc_len);
        mps_reader_zero(rd);
        rd->acc = acc;
        rd->acc_len = acc_len;
...
int mbedtls_mps_reader_feed(mbedtls_mps_reader* rd, unsigned char* new_frag, mbedtls_mps_size_t new_frag_len)
{
        mbedtls_mps_size_t copy_to_acc;
        MBEDTLS_MPS_TRACE_INIT("mbedtls_mps_reader_feed");
        MBEDTLS_MPS_TRACE(MBEDTLS_MPS_TRACE_TYPE_COMMENT, "* Fragment length: %u bytes", (unsigned)new_frag_len);

        if (new_frag == NULL)
                MBEDTLS_MPS_TRACE_RETURN(MBEDTLS_ERR_MPS_READER_INVALID_ARG);

        MBEDTLS_MPS_STATE_VALIDATE_RAW(mps_reader_is_producing(rd), "mbedtls_mps_reader_feed() requires reader to be "
                                                                    "in producing mode");
        ...
        if (mps_reader_is_accumulating(rd)) {
                ...
                MBEDTLS_MPS_TRACE(MBEDTLS_MPS_TRACE_TYPE_COMMENT, "Enough data available to serve user request");
...
int mbedtls_mps_reader_get(mbedtls_mps_reader* rd, mbedtls_mps_size_t desired, unsigned char** buffer,
                           mbedtls_mps_size_t* buflen)
{
        ...
        MBEDTLS_MPS_STATE_VALIDATE_RAW(mps_reader_is_consuming(rd), "mbedtls_mps_reader_get() requires reader to be in "
                                                                    "consuming mode");
        ...
        MBEDTLS_MPS_TRACE(MBEDTLS_MPS_TRACE_TYPE_COMMENT, "Serve the request from the current fragment.");
        ...
        if (frag_remaining < desired) {
                MBEDTLS_MPS_TRACE(MBEDTLS_MPS_TRACE_TYPE_COMMENT, "There's not enough data in the current fragment "
...
int mbedtls_mps_reader_commit(mbedtls_mps_reader* rd)
{
        ...
        MBEDTLS_MPS_STATE_VALIDATE_RAW(mps_reader_is_consuming(rd), "mbedtls_mps_reader_commit() requires reader to be "
                                                                    "in consuming mode");
...
int mbedtls_mps_reader_reclaim(mbedtls_mps_reader* rd, int* paused)
{
        ...
        MBEDTLS_MPS_STATE_VALIDATE_RAW(mps_reader_is_consuming(rd), "mbedtls_mps_reader_reclaim() requires reader to "
        ...
        if (pending == 0) {
                MBEDTLS_MPS_TRACE(MBEDTLS_MPS_TRACE_TYPE_COMMENT, "No unsatisfied read-request has been logged.");
                ...
                MBEDTLS_MPS_TRACE(MBEDTLS_MPS_TRACE_TYPE_COMMENT, "Fragment has been fully processed and committed.");
        } else {
                int overflow;
                ...
                if (overflow || acc_len < acc_len_needed) {
                        ...
                        rd->end = commit;
                        rd->pending = 0;
                        MBEDTLS_MPS_TRACE(MBEDTLS_MPS_TRACE_TYPE_ERROR, "The accumulator is too small to handle the "
                                                                        "backup.");
                ...
                MBEDTLS_MPS_TRACE(MBEDTLS_MPS_TRACE_TYPE_COMMENT, "Fragment backup: %u", (unsigned)frag_backup_len);
                MBEDTLS_MPS_TRACE(MBEDTLS_MPS_TRACE_TYPE_COMMENT, "Accumulator backup: %u", (unsigned)acc_backup_len);
