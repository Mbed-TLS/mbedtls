...
int mbedtls_nist_kw_setkey(mbedtls_nist_kw_context* ctx, mbedtls_cipher_id_t cipher, const unsigned char* key,
                           unsigned int keybits, const int is_wrap)
{
 ...
 */
int mbedtls_nist_kw_wrap(mbedtls_nist_kw_context* ctx, mbedtls_nist_kw_mode_t mode, const unsigned char* input,
                         size_t in_len, unsigned char* output, size_t* out_len, size_t out_size)
{
        int ret = 0;
        size_t semiblocks = 0;
        size_t s;
 ...
 */
int mbedtls_nist_kw_unwrap(mbedtls_nist_kw_context* ctx, mbedtls_nist_kw_mode_t mode, const unsigned char* input,
                           size_t in_len, unsigned char* output, size_t* out_len, size_t out_size)
{
        int ret = 0;
        size_t i, olen;
...
static const unsigned char kw_key[KW_TESTS][32] = {
        ...
        {0x11, 0x2a, 0xd4, 0x1b, 0x48, 0x56, 0xc7, 0x25, 0x4a, 0x98, 0x48, 0xd3, 0x0f, 0xdd, 0x78, 0x33,
         0x5b, 0x03, 0x9a, 0x48, 0xa8, 0x96, 0x2c, 0x4d, 0x1c, 0xb7, 0x8e, 0xab, 0xd5, 0xda, 0xd7, 0x88}};
...
static const unsigned char kw_msg[KW_TESTS][40] = {
        ...
        {0x95, 0xc1, 0x1b, 0xf5, 0x35, 0x3a, 0xfe, 0xdb, 0x98, 0xfd, 0xd6, 0xc8, 0xca, 0x6f,
         0xdb, 0x6d, 0xa5, 0x4b, 0x74, 0xb4, 0x99, 0x0f, 0xdc, 0x45, 0xc0, 0x9d, 0x15, 0x8f,
         0x51, 0xce, 0x62, 0x9d, 0xe2, 0xaf, 0x26, 0xe3, 0x25, 0x0e, 0x6b, 0x4c},
...
static const unsigned char kw_res[KW_TESTS][48] = {
        ...
        {0x44, 0x3c, 0x6f, 0x15, 0x09, 0x83, 0x71, 0x91, 0x3e, 0x5c, 0x81, 0x4c, 0xa1, 0xa0, 0x42, 0xec,
         0x68, 0x2f, 0x7b, 0x13, 0x6d, 0x24, 0x3a, 0x4d, 0x6c, 0x42, 0x6f, 0xc6, 0x97, 0x15, 0x63, 0xe8,
         0xa1, 0x4a, 0x55, 0x8e, 0x09, 0x64, 0x16, 0x19, 0xbf, 0x03, 0xfc, 0xaf, 0x90, 0xb1, 0xfc, 0x2d},
        {0xba, 0x8a, 0x25, 0x9a, 0x47, 0x1b, 0x78, 0x7d, 0xd5, 0xd5, 0x40, 0xec, 0x25, 0xd4, 0x3d, 0x87,
         0x20, 0x0f, 0xda, 0xdc, 0x6d, 0x1f, 0x05, 0xd9, 0x16, 0x58, 0x4f, 0xa9, 0xf6, 0xcb, 0xf5, 0x12}};
...
static const unsigned char kwp_msg[KW_TESTS][31] = {{0xbd, 0x68, 0x43, 0xd4, 0x20, 0x37, 0x8d, 0xc8, 0x96},
...
static const unsigned char kwp_res[KW_TESTS][48] = {
        ...
        {0x4e, 0x9b, 0xc2, 0xbc, 0xbc, 0x6c, 0x1e, 0x13, 0xd3, 0x35, 0xbc, 0xc0, 0xf7, 0x73,
         0x6a, 0x88, 0xfa, 0x87, 0x53, 0x66, 0x15, 0xbb, 0x8e, 0x63, 0x8b, 0xcc, 0x81, 0x66,
         0x84, 0x68, 0x17, 0x90, 0x67, 0xcf, 0xa9, 0x8a, 0x9d, 0x0e, 0x33, 0x26},
...
int mbedtls_nist_kw_self_test(int verbose)
{
        ...
        for (i = 0; i < KW_TESTS; i++) {
                ...
                ret = mbedtls_nist_kw_wrap(&ctx, MBEDTLS_KW_MODE_KW, kw_msg[i], kw_msg_len[i], out, &olen, sizeof(out));
                ...
                ret = mbedtls_nist_kw_unwrap(&ctx, MBEDTLS_KW_MODE_KW, out, olen, out, &olen, sizeof(out));
        ...
        for (i = 0; i < KW_TESTS; i++) {
                ...
                if (ret != 0 || kwp_out_len[i] != olen || memcmp(out, kwp_res[i], kwp_out_len[i]) != 0) {
                        if (verbose != 0)
                                mbedtls_printf("failed. ");
                ...
                ret = mbedtls_nist_kw_unwrap(&ctx, MBEDTLS_KW_MODE_KWP, out, olen, out, &olen, sizeof(out));

                if (ret != 0 || olen != kwp_msg_len[i] || memcmp(out, kwp_msg[i], kwp_msg_len[i]) != 0) {
                        if (verbose != 0)
                                mbedtls_printf("failed. ");
