                                   ...
                                   mbedtls_asn1_buf *salt, int *iterations)
{
    ...
    if ((ret =
             mbedtls_asn1_get_tag(p, end, &salt->len,
                       ...
                       unsigned char *output)
{
    ...
    if ((ret =
             mbedtls_cipher_set_iv(&cipher_ctx, iv, cipher_info->iv_size)) != 0)
        goto exit;
...
static int calculate_hashes(mbedtls_md_type_t md_type,
                            int iterations,
                            unsigned char *diversifier,
                            unsigned char *salt_block,
                            unsigned char *pwd_block,
                              ...
                              mbedtls_md_type_t md_type, int id, int iterations)
{
    ...
    while (datalen > 0) {
        if (calculate_hashes(md_type, iterations, diversifier, salt_block,
                             pwd_block, hash_output, use_salt, use_password,
                             hlen,
                             v) != 0) {
            goto exit;
        }
