...
exit:
    ...
    const psa_algorithm_t alg =
        PSA_ALG_HMAC(mbedtls_hash_info_psa_from_md(md_alg));
    ...
    while (key_length) {
        ...
        if ((status =
                 psa_mac_update(&operation, counter,
                                sizeof(counter))) != PSA_SUCCESS) {
            goto cleanup;
        }
        ...
        for (i = 1; i < iteration_count; i++) {
            ...
            if ((status =
                     psa_mac_update(&operation, md1, md_size)) != PSA_SUCCESS) {
                goto cleanup;
            }
            if ((status =
                     psa_mac_sign_finish(&operation, md1, out_size,
                                         &out_len)) != PSA_SUCCESS) {
                goto cleanup;
            }
