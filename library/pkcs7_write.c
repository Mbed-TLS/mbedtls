...
/*
 *Taken from MBEDTLS, mbedtls-mbedtls2.23.0/programs/util/pem2der.c many thanks
...
{
    size_t white_spaceSize = 0;
    while( white_spaceSize < data_size && data[white_spaceSize] == 0x00 )
        white_spaceSize++;
    return ( white_spaceSize );
}

/*
 * \brief 		given a data buffer it generates the desired hash
 * \param data, data to be hashed
 * \param size , length of buff
 * \param hash_funct, mbedtls_md_type, message digest type
 ...
 */
static int pkcs7_hash_create( const unsigned char *data, size_t size, int hash_funct,
           char **out_hash, size_t *out_hash_size )
{
    ...
    *out_hash = calloc( 1, md_info->size );
    if( !*out_hash ) {
        goto out;
    }

    rc = mbedtls_md_finish( &ctx, (unsigned char *) *out_hash );
    if( rc ) 
    {
...
/*
 *A general way to add data to the pkcs7 buffer from a given data type
 ...
 *to. memory from start to pointer should be unused. REMEMBER mbedtls writes
 *their buffers from the end of a buffer to the start
 ...
 *is length of data contained in the tag (not the length of the tag itself)
 * \param param, extra argument if adding algorthm identifier (tag =
 *MBEDTLS_ASN1_OID | MBEDTLS_ASN1_CONTEXT_SPECIFIC) to show the size of the
 *param buffer, 0 for NULL tag
 */
                        ...
                        size_t value_size, int param )
{
    ...
    do 
    {
        ...
        switch ( data_type )
        {
			...
			case WRITE_TAG :      
                rc = mbedtls_asn1_write_len( ptr, *start, value_size );
                if( rc >= 0 ) 
            	    rc = mbedtls_asn1_write_tag( ptr, *start, *(unsigned char *) value );
                break;

            case WRITE_OID :
            	rc = mbedtls_asn1_write_algorithm_identifier(
                     ptr, *start, (const char *) value, value_size, param );
            	break;

        	case WRITE_OID_NO_NULL_TAG :
                rc = mbedtls_asn1_write_oid( ptr, *start, (const char *) value, value_size );
                if( rc >= 0 ) 
                {
                    rc = mbedtls_asn1_write_len( ptr, *start, ptr_tmp - *ptr );
                    if( rc >= 0 ) 
                        rc = mbedtls_asn1_write_tag( ptr, *start, 
                    	    ( MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE) ); 
                }  
        		break;
        
            case WRITE_SIGNATURE :
                rc = mbedtls_asn1_write_octet_string( ptr, *start, value, value_size );
                break;

            case WRITE_RAW_BUFFER :
                rc = mbedtls_asn1_write_raw_buffer( ptr, *start, value, value_size );
                break;
        	
        	case WRITE_SERIAL :
                rc = mbedtls_asn1_write_tagged_string( ptr, *start, MBEDTLS_ASN1_INTEGER,
        ...
        {
            *ptr = ptr_tmp;  // set ptr back to old one, could have changed when
            // function failed
                        ...
                        size_t priv_size )
{
    ...
    priv_key = malloc( sizeof( *priv_key ) );
    if( !priv_key ) 
        return ( MBEDTLS_ERR_PKCS7_ALLOC_FAILED );

    mbedtls_pk_init( priv_key );
    // make sure private key parses into private key format
    rc = mbedtls_pk_parse_key( priv_key, priv, priv_size, NULL, 0 );
    if( rc ) 
        goto out;
    
    // make sure private key is matched with public key
    rc = mbedtls_pk_check_pair( &( pub->pk ), priv_key );
    if( rc ) 
        goto out;
    ...
    {
        rc = MBEDTLS_ERR_PKCS7_INVALID_SIGNER_INFO;
        goto out;
    }
    ...
    rc = pkcs7_hash_create( pkcs7_info->data, pkcs7_info->data_size, pkcs7_info->hash_funct,
                &hash, &hash_size );
    if( rc ) 
        goto out;
    ...
    rc = mbedtls_pk_sign( priv_key, pkcs7_info->hash_funct,
    ...
    rc = pkcs7_write_data( start, size, ptr, WRITE_SIGNATURE, signature,
                      sig_size, 0 );
    if( rc ) 
    {
        mbedtls_printf(
...
out:
    ...
    if( priv_key ) mbedtls_free( priv_key );
    if( hash ) mbedtls_free( hash );
    if( signature ) mbedtls_free( signature );
    return ( rc );
}
                           ...
                           size_t priv_size )
{
    ...
    rc = pkcs7_set_signature( start, size, ptr, pkcs7_info, pub, priv, priv_size );
    if( !rc ) 
    {
        ...
        if( strcmp( sig_type, "RSA" ) ) 
        {
            rc = MBEDTLS_ERR_PKCS7_INVALID_CERT;
...
static int pkcs7_set_signer_cert_data( unsigned char **start, size_t *size,
                             ...
                             size_t priv_size )
{
    int rc, signed_info_version = 1;
    size_t bytes_written_in_step, curr_len;
    unsigned char tag = MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE;
    rc = pkcs7_set_algorithm_ids( start, size, ptr, pkcs7_info, pub, priv, priv_size );
    if( !rc ) 
    {
        // add serial
        curr_len = *size - ( *ptr - *start );
        rc = pkcs7_write_data( start, size, ptr,
                          WRITE_SERIAL,
                          pub->serial.p, pub->serial.len, 0 );
        if( !rc ) 
        {
            rc = pkcs7_write_data( start, size, ptr, WRITE_RAW_BUFFER,
            ...
            if( !rc ) 
            {
                ...
                if( !rc ) 
                {
                    ...
                    // section 9.2
                    rc = pkcs7_write_data( start, size, ptr, WRITE_VERSION,
                                      &signed_info_version, sizeof( signed_info_version ), 0 );
                }
                if( rc ) 
                {
                    mbedtls_printf(
                        "ERROR: Failed to add owner flag to Signer Info of PKCS7\n");
                }
            } 
            else
                mbedtls_printf(
                    "ERROR: Failed to add issuer data to Signer info of PKCS7\n");
        } 
        else
            mbedtls_printf(
                "ERROR: Failed to add certificate serial number to Signer info of "
                "PKCS7\n");
    }
    return ( rc );
}
                                      ...
                                      mbedtls_pkcs7_info *pkcs7_info )
{
    ...
    for( int i = 0; i < pkcs7_info->key_pairs; i++ ) 
    {
        ...
        rc = mbedtls_x509_crt_parse( x509, pkcs7_info->crts[i],
        ...
        curr_len = *size - ( *ptr - *start );
        rc = pkcs7_set_signer_cert_data( start, size, ptr, pkcs7_info, x509,
                          ...
                          unsigned char **ptr, mbedtls_pkcs7_info *pkcs7_info )
{
    ...
    curr_len = *size - ( *ptr - *start );
    rc = pkcs7_set_each_signer_data( start, size, ptr, pkcs7_info );
    if( !rc ) 
    {
        bytes_written_in_step = *size - ( *ptr - *start ) - curr_len;
                            ...
                            unsigned char **ptr, mbedtls_pkcs7_info *pkcs7_info )
{
    ...
    rc = pkcs7_set_signers_data( start, size, ptr, pkcs7_info );
    ...
    if( !rc ) 
    {
        ...
        {
            bytes_written_in_step = *size - ( *ptr - *start ) - curr_len;
            rc = pkcs7_write_data( start, size, ptr, WRITE_TAG,
...
{
    int rc;
    rc = pkcs7_set_signer_certs( start, size, ptr, pkcs7_info );
    if( !rc ) 
    {
        rc = pkcs7_write_data( start, size, ptr, WRITE_OID_NO_NULL_TAG,
...
static int pkcs7_set_algo_id( unsigned char **start, size_t *size, unsigned char **ptr,
                     mbedtls_pkcs7_info *pkcs7_info )
{
    int rc;
    unsigned char tag = MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SET;
    size_t bytes_written_in_step, curr_len;
    rc = pkcs7_set_signed_data_oid( start, size, ptr, pkcs7_info );

    if( !rc ) 
    {
        curr_len = *size - ( *ptr - *start );

        rc = pkcs7_write_data( start, size, ptr, WRITE_OID, 
        ...
        if( !rc ) 
        {
            ...
            // bytes
            bytes_written_in_step = *size - ( *ptr - *start ) - curr_len;
            rc = pkcs7_write_data( start, size, ptr, WRITE_TAG, &tag,
                              bytes_written_in_step, 0 );
        }
        if( rc ) 
...
static int pkcs7_set_version( unsigned char **start, size_t *size, unsigned char **ptr,
                      mbedtls_pkcs7_info *pkcs7_info )
{
    int rc;
    int version = 1;
    // for now only version 1
    rc = pkcs7_set_algo_id( start, size, ptr, pkcs7_info );
    if( !rc ) 
    {
        rc = pkcs7_write_data( start, size, ptr, WRITE_VERSION, &version,
                          sizeof( version ), 0 );
        if( rc ) 
                         ...
                         unsigned char **ptr, mbedtls_pkcs7_info *pkcs7_info )
{
    ...
    size_t curr_len;
    unsigned char tag = MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE;
    rc = pkcs7_set_version( start, size, ptr, pkcs7_info );
    if( !rc ) 
    {
        curr_len = *size - ( *ptr - *start );
        rc = pkcs7_write_data( start, size, ptr,
...
static int pkcs7_set_pkcs7_oid( unsigned char **start, size_t *size, unsigned char **ptr,
                       mbedtls_pkcs7_info *pkcs7_info )
{
    int rc;
    size_t curr_len;
    unsigned char tag = MBEDTLS_ASN1_CONTEXT_SPECIFIC | MBEDTLS_ASN1_CONSTRUCTED;
    rc = pkcs7_set_signed_data( start, size, ptr, pkcs7_info );
					...
					unsigned char **ptr, mbedtls_pkcs7_info *pkcs7_info )
{
	...
	//rip stack memory but writing backwards gives me a headache
            ...
            int hash_funct )
{
    ...
    if( hash_funct < MBEDTLS_MD_NONE || hash_funct > MBEDTLS_MD_RIPEMD160 ) 
    {
        mbedtls_printf( "ERROR: Invalid hash function %d, see mbedtls_md_type_t\n",
                       hash_funct );
        rc = MBEDTLS_ERR_PKCS7_INVALID_ALG;
        goto out;
    }
    rc = mbedtls_oid_get_oid_by_md( hash_funct, (const char **) &hash_funct_oid,
                                   &oid_len );
    if( rc ) 
    {
        mbedtls_printf(
    ...
    rc = pkcs7_start_generation( (unsigned char **) &pkcs7_buff, &pkcs7_buff_size, &ptr, &info );
    if( rc ) 
    {
    ...
    pkcs7_buff_size -= white_space;  // factor off leading whitespace from pkcs7

    // copy into new buffer with only necessary allocated memory
    *pkcs7_size = pkcs7_buff_size;
    *pkcs7 = malloc( *pkcs7_size );
    if( !*pkcs7 ) 
    {
        rc = MBEDTLS_ERR_PKCS7_ALLOC_FAILED;
        goto out;
