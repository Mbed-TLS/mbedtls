                        ...
                        size_t priv_size )
{
    ...
    ret = pkcs7_hash_create( pkcs7_info->data, pkcs7_info->data_size, pkcs7_info->hash_funct,
                &hash, &hash_size );
    if( ret != 0 ) 
        goto out;
                             ...
                             size_t priv_size )
{
    ...
    ret = pkcs7_set_algorithm_ids( start, size, ptr, pkcs7_info, pub, priv, priv_size );
    if( ret != 0 ) 
        return ( ret );
            ...
            mbedtls_md_type_t hash_funct )
{
    ...
    ret = pkcs7_start_generation( (unsigned char **) &pkcs7_buff, &pkcs7_buff_size, &ptr, &info );
    if( ret ) 
    {
