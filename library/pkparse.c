                                   ...
                                   mbedtls_ecp_group *grp)
{
     ...
     */
    if ((ret =
             mbedtls_asn1_get_tag(&p, end_field, &len, MBEDTLS_ASN1_OID)) != 0)
        return(ret);
     ...
     */
    if ((ret =
             mbedtls_asn1_get_tag(&p, end_curve, &len,
                                  ...
                                  mbedtls_ecp_group_id *grp_id)
{
    ...
    for (id = mbedtls_ecp_grp_id_list(); *id != MBEDTLS_ECP_DP_NONE; id++) {
        ...
        if (grp->pbits == ref.pbits && grp->nbits == ref.nbits &&
            ...
            mbedtls_mpi_get_bit(&grp->G.Y,
                                0) == mbedtls_mpi_get_bit(&ref.G.Y, 0)) {
            break;
        }
    ...
    int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    ...
    if (pk_alg == MBEDTLS_PK_ECKEY || pk_alg == MBEDTLS_PK_ECKEY_DH) {
        if ((ret = pk_use_ecparams(&params, &mbedtls_pk_ec(*pk)->grp)) != 0 ||
            (ret =
                 pk_parse_key_sec1_der(mbedtls_pk_ec(*pk), p, len, f_rng,
                                       p_rng)) != 0) {
            mbedtls_pk_free(pk);
            return(ret);
        }
    ...
    int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    ...
    if (MBEDTLS_OID_CMP(MBEDTLS_OID_PKCS5_PBES2, &pbe_alg_oid) == 0) {
        if ((ret =
                 mbedtls_pkcs5_pbes2(&pbe_params, MBEDTLS_PKCS5_DECRYPT, pwd,
                                     pwdlen,
                                     p, len, buf)) != 0) {
                                ...
                                const unsigned char *key, size_t keylen)
{
    ...
    if (ret == 0) {
        ...
        if ((ret =
                 pk_get_rsapubkey(&p, p + pem.buflen,
