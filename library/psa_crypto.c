    ...
    mbedtls_svc_key_id_t     *key)
{
    ...
    if (driver != NULL &&
        psa_crypto_transaction.unknown.type ==
    ...
    size_t                     output_length)
{
    ...
    status = psa_hash_compute(PSA_ALG_SHA_256, ecjpake->data,
                              PSA_TLS12_ECJPAKE_TO_PMS_DATA_SIZE, output,
                              output_length,
                              &output_size);
    if (status != PSA_SUCCESS) {
        return status;
    ...
    )
{
    ...
    *data = mbedtls_calloc(1, m_bytes);
    ...
    while (key_out_of_range) {
        /* 1. Draw a byte string of length ceiling(m/8) bytes. */
        if ((status =
                 psa_key_derivation_output_bytes(operation, *data,
                                                 m_bytes)) != 0) {
            goto cleanup;
        }
         ...
         */
        MBEDTLS_MPI_CHK(mbedtls_mpi_lt_mpi_ct(&diff_N_2, &k,
                                              &key_out_of_range));
    }
    ...
    psa_key_derivation_operation_t *operation)
{
    ...
    if (PSA_KEY_TYPE_IS_ECC(slot->attr.type)) {
        ...
        if (PSA_ECC_FAMILY_IS_WEIERSTRASS(curve)) {
            ...
            status = psa_generate_derived_ecc_key_weierstrass_helper(slot, bits,
                                                                     operation,
                                                                     &data);
            if (status != PSA_SUCCESS) {
                goto exit;
            ...
            status = psa_generate_derived_ecc_key_montgomery_helper(bits,
                                                                    operation,
                                                                    &data);
            if (status != PSA_SUCCESS) {
                goto exit;
    ...
    if (prf->state == PSA_TLS12_PRF_STATE_OTHER_KEY_SET) {
        ...
        *cur++ = MBEDTLS_BYTE_0(prf->other_secret_length);
        if (prf->other_secret_length != 0) {
            ...
            mbedtls_platform_zeroize(prf->other_secret,
                                     prf->other_secret_length);
