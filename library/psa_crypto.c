...
psa_status_t psa_key_derivation_abort( psa_key_derivation_operation_t *operation )
{
        ...
        PSA_ALG_IS_HKDF_EXPAND( kdf_alg ) )
    {
        ...
        if ( is_input_stage )
        {
            ...
            if( tls12_prf_inputs->seed != NULL )
            {
                mbedtls_platform_zeroize( tls12_prf_inputs->seed,
                    tls12_prf_inputs->seed_length );
                mbedtls_free( tls12_prf_inputs->seed );
            }
             ...
             PSA_ALG_IS_TLS12_PSK_TO_MS( kdf_alg ) )
    {
        ...
        uint8_t *other_secret = is_input_stage ?
            tls12_prf_inputs->other_secret : tls12_prf_context->other_secret;
        size_t other_secret_length = is_input_stage ?
            tls12_prf_inputs->other_secret_length : tls12_prf_context->other_secret_length;
        ...
        switch( step )
        {
            case PSA_KEY_DERIVATION_INPUT_SEED:
                if ( PSA_ALG_IS_HKDF_EXPAND( kdf_alg ) )
                    return ( PSA_ERROR_INVALID_ARGUMENT );
                if( buffer_size < hkdf_inputs->seed_length )
                    return PSA_ERROR_BUFFER_TOO_SMALL;
                memcpy( buffer, hkdf_inputs->seed,
                        hkdf_inputs->seed_length );
                *buffer_length = hkdf_inputs->seed_length;
                break;
            case PSA_KEY_DERIVATION_INPUT_SECRET:
    ...
    psa_key_derivation_operation_t *operation )
{
    ...
    uint8_t  *secret = NULL, *seed = NULL, *info = NULL;
    size_t secret_length = 0, seed_length = 0, info_length = 0;
    ...
    psa_key_derivation_operation_t *operation )
{
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
    size_t other_secret_length = 0;
    uint8_t *other_secret = NULL;
#endif /* MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS */
    ...
    if ( ( status = psa_crypto_driver_key_derivation_get_input_size(
                        ...
                        &other_secret_length ) != PSA_SUCCESS ) )
        return status;
#endif /* MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS */
    ...
    if( other_secret_length != 0 )
        other_secret = mbedtls_calloc( 1, other_secret_length );
#endif /* MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS */
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
        || ( other_secret_length != 0 && other_secret == NULL )
#endif /* MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS */
      )
    {
        mbedtls_free( seed );
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
    tls12_prf->other_secret = other_secret;
    tls12_prf->other_secret_length = other_secret_length;
#endif /* MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS */
...
static psa_status_t psa_hkdf_prf_set_seed( psa_hkdf_key_derivation_inputs_t *hkdf,
                                            const uint8_t *data,
                                            size_t data_length )
{
    ...
    if( data_length != 0 )
    {
        hkdf->seed = mbedtls_calloc( 1, data_length );
        if( hkdf->seed == NULL )
            return( PSA_ERROR_INSUFFICIENT_MEMORY );

        memcpy( hkdf->seed, data, data_length );
        hkdf->seed_length = data_length;
    }
                                    ...
                                    size_t data_length )
{
    ...
    switch( step )
    {
        case PSA_KEY_DERIVATION_INPUT_SALT:
            if( PSA_ALG_IS_HKDF_EXPAND( kdf_alg ) )
                return( PSA_ERROR_INVALID_ARGUMENT );
            return ( psa_hkdf_prf_set_seed( hkdf, data, data_length ) );
        case PSA_KEY_DERIVATION_INPUT_SECRET:
