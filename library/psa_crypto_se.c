...
static psa_status_t psa_get_se_driver_its_file_uid(const psa_se_drv_table_entry_t* driver, psa_storage_uid_t* uid)
{
...
psa_status_t psa_save_se_persistent_data(const psa_se_drv_table_entry_t* driver)
{
         ...
         * persistent_data_size is in range, but compilers don't know that,
         * so cast to reassure them. */
        return (psa_its_set(uid, (uint32_t)driver->u.internal.persistent_data_size, driver->u.internal.persistent_data,
                            0));
}
...
psa_status_t psa_find_se_slot_for_key(const psa_key_attributes_t* attributes, psa_key_creation_method_t method,
                                      psa_se_drv_table_entry_t* driver, psa_key_slot_number_t* slot_number)
{
        psa_status_t status;
        ...
        if (psa_get_key_slot_number(attributes, slot_number) == PSA_SUCCESS) {
                ...
                if (p_validate_slot_number == NULL)
                        return (PSA_ERROR_NOT_SUPPORTED);
                status = p_validate_slot_number(&driver->u.context, driver->u.internal.persistent_data, attributes,
                                                method, *slot_number);
                ...
                if (p_allocate == NULL)
                        return (PSA_ERROR_NOT_SUPPORTED);
                status = p_allocate(&driver->u.context, driver->u.internal.persistent_data, attributes, method,
                                    slot_number);
        }
        return (status);
}

psa_status_t psa_destroy_se_key(psa_se_drv_table_entry_t* driver, psa_key_slot_number_t slot_number)
{
        psa_status_t status;
        ...
        status = driver->methods->key_management->p_destroy(&driver->u.context, driver->u.internal.persistent_data,
                                                            slot_number);
...
psa_status_t psa_init_all_se_drivers(void)
{
        ...
        for (i = 0; i < PSA_MAX_SE_DRIVERS; i++) {
                ...
                if (methods->p_init != NULL) {
                        psa_status_t status = methods->p_init(&driver->u.context, driver->u.internal.persistent_data,
                                                              driver->location);
                        if (status != PSA_SUCCESS)
                                return (status);
