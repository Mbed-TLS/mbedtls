...
psa_status_t psa_get_empty_key_slot(psa_key_id_t* volatile_key_id, psa_key_slot_t** p_slot)
{
        ...
        for (slot_idx = 0; slot_idx < MBEDTLS_PSA_KEY_SLOT_COUNT; slot_idx++) {
                ...
                if ((unlocked_persistent_key_slot == NULL) && (!PSA_KEY_LIFETIME_IS_VOLATILE(slot->attr.lifetime)) &&
...
static psa_status_t psa_load_persistent_key_into_slot(psa_key_slot_t* slot)
{
        ...
        if (psa_get_se_driver(slot->attr.lifetime, &drv, &drv_context)) {
                ...
                status = psa_copy_key_material_into_slot(slot, data->slot_number, sizeof(data->slot_number));
                goto exit;
        }
