...
static int ssl_write_client_certificate_ext( mbedtls_ssl_context *ssl,
                                              unsigned char *buf,
                                              size_t *olen )
{
    unsigned char *p = buf;
...
static int ssl_write_server_certificate_ext( mbedtls_ssl_context *ssl,
                                              unsigned char *buf,
                                              size_t *olen )
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_SSL_RAW_PUBLIC_KEY_SUPPORT)
    if( ( ret = ssl_write_client_certificate_ext( ssl, p + 2 + ext_len, &olen ) ) != 0 )
    {
       MBEDTLS_SSL_DEBUG_RET( 1, "ssl_write_client_certificate_ext", ret );
       return( ret );
    }
    ext_len += olen;

    if( ( ret = ssl_write_server_certificate_ext( ssl, p + 2 + ext_len, &olen )) != 0 )
    {
        MBEDTLS_SSL_DEBUG_RET( 1, "ssl_write_server_certificate_ext", ret );
            return( ret );
    }
    ext_len += olen;
#endif
