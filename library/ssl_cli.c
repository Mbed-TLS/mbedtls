...
static int ssl_write_hostname_ext(mbedtls_ssl_context* ssl, unsigned char* buf, const unsigned char* end, size_t* olen)
{
        unsigned char* p = buf;
        size_t hostname_len;
...
static int ssl_write_renegotiation_ext(mbedtls_ssl_context* ssl, unsigned char* buf, const unsigned char* end,
                                       size_t* olen)
{
        unsigned char* p = buf;
...
static int ssl_write_signature_algorithms_ext(mbedtls_ssl_context* ssl, unsigned char* buf, const unsigned char* end,
                                              size_t* olen)
{
        unsigned char* p = buf;
        ...
        *olen = 0;
        ...
        for (md = ssl->conf->sig_hashes; *md != MBEDTLS_MD_NONE; md++) {
                ...
                if (sig_alg_len > MBEDTLS_SSL_MAX_SIG_HASH_ALG_LIST_LEN) {
                        MBEDTLS_SSL_DEBUG_MSG(3, ("length in bytes of sig-hash-alg extension too big"));
                        return (MBEDTLS_ERR_SSL_BAD_CONFIG);
                }
        }
...
static int ssl_write_supported_elliptic_curves_ext(mbedtls_ssl_context* ssl, unsigned char* buf,
...
static int ssl_write_supported_point_formats_ext(mbedtls_ssl_context* ssl, unsigned char* buf, const unsigned char* end,
                                                 size_t* olen)
{
        unsigned char* p = buf;
...
static int ssl_write_ecjpake_kkpp_ext(mbedtls_ssl_context* ssl, unsigned char* buf, const unsigned char* end,
                                      size_t* olen)
{
        ...
        *p++ = (unsigned char)((MBEDTLS_TLS_EXT_ECJPAKE_KKPP)&0xFF);
        ...
        if (ssl->handshake->ecjpake_cache == NULL || ssl->handshake->ecjpake_cache_len == 0) {
                ...
                ret = mbedtls_ecjpake_write_round_one(&ssl->handshake->ecjpake_ctx, p + 2, end - p - 2, &kkpp_len,
...
static int ssl_write_cid_ext(mbedtls_ssl_context* ssl, unsigned char* buf, const unsigned char* end, size_t* olen)
{
        unsigned char* p = buf;
        size_t ext_len;
        ...
        *olen = 0;
        if (ssl->conf->transport != MBEDTLS_SSL_TRANSPORT_DATAGRAM || ssl->negotiate_cid == MBEDTLS_SSL_CID_DISABLED) {
                return (0);
        }
...
static int ssl_write_max_fragment_length_ext(mbedtls_ssl_context* ssl, unsigned char* buf, const unsigned char* end,
                                             size_t* olen)
{
        unsigned char* p = buf;
...
static int ssl_write_encrypt_then_mac_ext(mbedtls_ssl_context* ssl, unsigned char* buf, const unsigned char* end,
                                          size_t* olen)
{
        unsigned char* p = buf;
...
static int ssl_write_extended_ms_ext(mbedtls_ssl_context* ssl, unsigned char* buf, const unsigned char* end,
                                     size_t* olen)
{
        unsigned char* p = buf;
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
static int ssl_write_session_ticket_ext(mbedtls_ssl_context* ssl, unsigned char* buf, const unsigned char* end,
                                        size_t* olen)
{
        unsigned char* p = buf;
...
static int ssl_write_alpn_ext(mbedtls_ssl_context* ssl, unsigned char* buf, const unsigned char* end, size_t* olen)
{
        unsigned char* p = buf;
        size_t alpnlen = 0;
        const char** cur;
...
static int ssl_write_use_srtp_ext(mbedtls_ssl_context* ssl, unsigned char* buf, const unsigned char* end, size_t* olen)
{
        unsigned char* p = buf;
        ...
        *p++ = (unsigned char)((2 * ssl->conf->dtls_srtp_profile_list_len) & 0xFF);
        ...
        for (protection_profiles_index = 0; protection_profiles_index < ssl->conf->dtls_srtp_profile_list_len;
             protection_profiles_index++) {
                ...
                if (profile_value != MBEDTLS_TLS_SRTP_UNSET) {
                        ...
                        *p++ = (profile_value & 0xFF);
                         ...
                         */
                        MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, "
...
static int ssl_generate_random(mbedtls_ssl_context* ssl)
{
        ...
        *p++ = (unsigned char)(t);
        ...
        MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, current time: %" MBEDTLS_PRINTF_LONGLONG, (long long)t));
#else
 ...
 */
static int ssl_validate_ciphersuite(const mbedtls_ssl_ciphersuite_t* suite_info, const mbedtls_ssl_context* ssl,
...
static int ssl_write_client_hello(mbedtls_ssl_context* ssl)
{
        ...
        if (ssl->conf->max_major_ver == 0) {
                MBEDTLS_SSL_DEBUG_MSG(1, ("configured max major version is invalid, consider using "
        ...
        *p++ = (unsigned char)n;
        ...
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
                ...
                if (ssl->handshake->verify_cookie == NULL) {
                        ...
                        *p++ = 0;
                        ...
                        MBEDTLS_SSL_DEBUG_BUF(3, "client hello, cookie", ssl->handshake->verify_cookie,
        ...
        for (i = 0; ciphersuites[i] != 0; i++) {
                ...
                if (ssl_validate_ciphersuite(ciphersuite_info, ssl, ssl->conf->min_minor_ver,
                                             ssl->conf->max_minor_ver) != 0)
                        continue;

                MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, add ciphersuite: %#04x (%s)", (unsigned int)ciphersuites[i],
        ...
        MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, got %" MBEDTLS_PRINTF_SIZET " ciphersuites (excluding SCSVs)", n));
        ...
        *p++ = MBEDTLS_SSL_COMPRESS_NULL;
...
#if defined(MBEDTLS_SSL_RENEGOTIATION)
        if ((ret = ssl_write_renegotiation_ext(ssl, p + 2 + ext_len, end, &olen)) != 0) {
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2) && defined(MBEDTLS_KEY_EXCHANGE_WITH_CERT_ENABLED)
        if ((ret = ssl_write_signature_algorithms_ext(ssl, p + 2 + ext_len, end, &olen)) != 0) {
...
#if defined(MBEDTLS_ECDH_C) || defined(MBEDTLS_ECDSA_C) || defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
        if (uses_ec) {
                if ((ret = ssl_write_supported_elliptic_curves_ext(ssl, p + 2 + ext_len, end, &olen)) != 0) {
                        MBEDTLS_SSL_DEBUG_RET(1, "ssl_write_supported_elliptic_curves_ext", ret);
                        return (ret);
                }
                ext_len += olen;

                if ((ret = ssl_write_supported_point_formats_ext(ssl, p + 2 + ext_len, end, &olen)) != 0) {
...
#if defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
        if ((ret = ssl_write_ecjpake_kkpp_ext(ssl, p + 2 + ext_len, end, &olen)) != 0) {
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
        if ((ret = ssl_write_max_fragment_length_ext(ssl, p + 2 + ext_len, end, &olen)) != 0) {
...
#if defined(MBEDTLS_SSL_ENCRYPT_THEN_MAC)
        if ((ret = ssl_write_encrypt_then_mac_ext(ssl, p + 2 + ext_len, end, &olen)) != 0) {
...
#if defined(MBEDTLS_SSL_EXTENDED_MASTER_SECRET)
        if ((ret = ssl_write_extended_ms_ext(ssl, p + 2 + ext_len, end, &olen)) != 0) {
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
        if ((ret = ssl_write_session_ticket_ext(ssl, p + 2 + ext_len, end, &olen)) != 0) {
        ...
        MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, total extension length: %" MBEDTLS_PRINTF_SIZET, ext_len));
...
static int ssl_parse_renegotiation_info(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len)
{
        ...
        if (ssl->renego_status != MBEDTLS_SSL_INITIAL_HANDSHAKE) {
                ...
                if (len != 1 + ssl->verify_data_len * 2 || buf[0] != ssl->verify_data_len * 2 ||
                    ...
                    mbedtls_ssl_safer_memcmp(buf + 1 + ssl->verify_data_len, ssl->peer_verify_data,
...
static int ssl_parse_use_srtp_ext(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len)
{
         ...
         */
        if (len > 5 && (buf[4] != mki_len || (memcmp(ssl->dtls_srtp_info.mki_value, &buf[5], mki_len)))) {
                mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                               MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER);
                return (MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER);
        }
...
static int ssl_parse_hello_verify_request(mbedtls_ssl_context* ssl)
{
         ...
         */
        if (major_ver < MBEDTLS_SSL_MAJOR_VERSION_3 || minor_ver < MBEDTLS_SSL_MINOR_VERSION_2 ||
...
static int ssl_parse_server_hello(mbedtls_ssl_context* ssl)
{
        ...
        if (ssl->major_ver < ssl->conf->min_major_ver || ssl->minor_ver < ssl->conf->min_minor_ver ||
            ssl->major_ver > ssl->conf->max_major_ver || ssl->minor_ver > ssl->conf->max_minor_ver) {
                MBEDTLS_SSL_DEBUG_MSG(1, ("server version out of bounds -  min: [%d:%d], server: [%d:%d], max: [%d:%d]",
                                          ssl->conf->min_major_ver, ssl->conf->min_minor_ver, ssl->major_ver,
                                          ssl->minor_ver, ssl->conf->max_major_ver, ssl->conf->max_minor_ver));
        ...
        if (ssl->handshake->resume == 0 || n == 0 ||
            ...
            ssl->session_negotiate->id_len != n || memcmp(ssl->session_negotiate->id, buf + 35, n) != 0) {
                ssl->state++;
                ssl->handshake->resume = 0;
#if defined(MBEDTLS_HAVE_TIME)
        ...
        MBEDTLS_SSL_DEBUG_MSG(2, ("server hello, total extension length: %" MBEDTLS_PRINTF_SIZET, ext_len));
        ...
        if (ssl->secure_renegotiation == MBEDTLS_SSL_LEGACY_RENEGOTIATION &&
            ssl->conf->allow_legacy_renegotiation == MBEDTLS_SSL_LEGACY_BREAK_HANDSHAKE) {
                ...
                handshake_failure = 1;
                 ...
                 ssl->secure_renegotiation == MBEDTLS_SSL_SECURE_RENEGOTIATION && renegotiation_info_seen == 0) {
                ...
                handshake_failure = 1;
                   ...
                   ssl->secure_renegotiation == MBEDTLS_SSL_LEGACY_RENEGOTIATION && renegotiation_info_seen == 1) {
...
static int ssl_parse_server_dh_params(mbedtls_ssl_context* ssl, unsigned char** p, unsigned char* end)
{
        ...
        if (dhm_actual_bitlen < ssl->conf->dhm_min_bitlen) {
                MBEDTLS_SSL_DEBUG_MSG(1, ("DHM prime too short: %" MBEDTLS_PRINTF_SIZET " < %u", dhm_actual_bitlen,
        ...
        defined(MBEDTLS_KEY_EXCHANGE_ECDHE_PSK_ENABLED) || defined(MBEDTLS_KEY_EXCHANGE_ECDH_RSA_ENABLED) || \
...
static int ssl_parse_server_ecdh_params_psa(mbedtls_ssl_context* ssl, unsigned char** p, unsigned char* end)
{
        uint16_t tls_id;
        size_t ecdh_bits = 0;
        uint8_t ecpoint_len;
        ...
        if (mbedtls_psa_tls_ecpoint_to_psa_ec(*p, ecpoint_len, handshake->ecdh_psa_peerkey,
...
static int ssl_write_encrypted_pms(mbedtls_ssl_context* ssl, size_t offset, size_t* olen, size_t pms_offset)
{
        ...
        if ((ret = mbedtls_pk_encrypt(peer_pk, p, ssl->handshake->pmslen, ssl->out_msg + offset + len_bytes, olen,
...
static int ssl_parse_signature_algorithm(mbedtls_ssl_context* ssl, unsigned char** p, unsigned char* end,
                                         mbedtls_md_type_t* md_alg, mbedtls_pk_type_t* pk_alg)
{
        ...
        *pk_alg = MBEDTLS_PK_NONE;
        ...
        if (mbedtls_ssl_check_sig_hash(ssl, *md_alg) != 0) {
                MBEDTLS_SSL_DEBUG_MSG(1, ("server used HashAlgorithm %d that was not offered", *(p)[0]));
                return (MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE);
        }
...
#if defined(MBEDTLS_KEY_EXCHANGE_ECDH_RSA_ENABLED) || defined(MBEDTLS_KEY_EXCHANGE_ECDH_ECDSA_ENABLED)
...
#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
        if (ssl->handshake->ecrs_enabled && ssl->handshake->ecrs_state == ssl_ecrs_ske_start_processing) {
...
start_processing:
        ...
        (defined(MBEDTLS_KEY_EXCHANGE_ECDHE_RSA_ENABLED) || defined(MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA_ENABLED))
            ...
            ( MBEDTLS_KEY_EXCHANGE_ECDHE_RSA_ENABLED || \
              MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA_ENABLED ) */
#if defined(MBEDTLS_KEY_EXCHANGE_ECDHE_RSA_ENABLED) || defined(MBEDTLS_KEY_EXCHANGE_ECDHE_PSK_ENABLED) || \
        defined(MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA_ENABLED)
                if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_RSA ||
                    ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK ||
                    ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA) {
        ...
        if (mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info)) {
                ...
                if (md_alg != MBEDTLS_MD_NONE) {
                        ret = mbedtls_ssl_get_key_exchange_md_tls1_2(ssl, hash, &hashlen, params, params_len, md_alg);
                        if (ret != 0)
                                return (ret);
                } else
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
                ...
                if ((ret = mbedtls_pk_verify_restartable(peer_pk, md_alg, hash, hashlen, p, sig_len, rs_ctx)) != 0) {
#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
                        if (ret != MBEDTLS_ERR_ECP_IN_PROGRESS)
#endif
                                mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL,
...
static int ssl_parse_certificate_request(mbedtls_ssl_context* ssl)
{
        ...
        if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3) {
                size_t sig_alg_len =
                        ((buf[mbedtls_ssl_hs_hdr_len(ssl) + 1 + n] << 8) | (buf[mbedtls_ssl_hs_hdr_len(ssl) + 2 + n]));
#if defined(MBEDTLS_DEBUG_C)
                ...
                for (i = 0; i < sig_alg_len; i += 2) {
                        MBEDTLS_SSL_DEBUG_MSG(3, ("Supported Signature Algorithm found: %d,%d", sig_alg[i],
                                                  sig_alg[i + 1]));
                }
#endif
...
static int ssl_write_client_key_exchange(mbedtls_ssl_context* ssl)
{
        ...
        (defined(MBEDTLS_KEY_EXCHANGE_ECDHE_RSA_ENABLED) || defined(MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA_ENABLED))
                ...
                status = psa_export_public_key(handshake->ecdh_psa_privkey, own_pubkey, sizeof(own_pubkey),
                                               &own_pubkey_len);
                if (status != PSA_SUCCESS)
                        return (MBEDTLS_ERR_SSL_HW_ACCEL_FAILED);

                if (mbedtls_psa_tls_psa_ec_to_ecpoint(own_pubkey, own_pubkey_len, &own_pubkey_ecpoint,
                ...
                memcpy(ssl->out_msg + header_len + 1, own_pubkey_ecpoint, own_pubkey_ecpoint_len);
        ...
        defined(MBEDTLS_KEY_EXCHANGE_ECDH_RSA_ENABLED) || defined(MBEDTLS_KEY_EXCHANGE_ECDH_ECDSA_ENABLED)
                ...
                ret = mbedtls_ecdh_make_public(&ssl->handshake->ecdh_ctx, &content_len, &ssl->out_msg[header_len], 1000,
                                               ssl->conf->f_rng, ssl->conf->p_rng);
                if (ret != 0) {
        ...
        ecdh_calc_secret:
          ...
          MBEDTLS_KEY_EXCHANGE_ECDH_ECDSA_ENABLED */
                ...
                if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_PSK &&
                    ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3 && ssl_conf_has_static_raw_psk(ssl->conf) == 0) {
          ...
          MBEDTLS_KEY_EXCHANGE_PSK_ENABLED */
                ...
                ret = mbedtls_ecjpake_write_round_two(&ssl->handshake->ecjpake_ctx, ssl->out_msg + header_len,
                ...
                ret = mbedtls_ecjpake_derive_secret(&ssl->handshake->ecjpake_ctx, ssl->handshake->premaster, 32,
...
static int ssl_parse_new_session_ticket(mbedtls_ssl_context* ssl)
{
        ...
        lifetime = (((uint32_t)msg[0]) << 24) | (msg[1] << 16) | (msg[2] << 8) | (msg[3]);
