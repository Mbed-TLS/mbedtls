...
static int ssl_write_trusted_ca_keys_ext( mbedtls_ssl_context *ssl,
                                          unsigned char *buf,
                                          const unsigned char *end,
                                          size_t *olen )
{
    ...
    *olen = 0;
    ...
    MBEDTLS_SSL_DEBUG_MSG( 3,
        ( "client hello, adding trusted_ca_keys extension" ) );
    ...
    {
        /* add 2 bytes for length, 1 byte for id_type */
        trusted_ca_keys_len += ( next_auth->len + 3 );
        next_auth = next_auth->next;
    }

    MBEDTLS_SSL_CHK_BUF_PTR( p, end, trusted_ca_keys_len + 4 );
    ...
    *p++ = (unsigned char)( ( ( trusted_ca_keys_len )      ) & 0xFF );
    ...
    while( next_auth != NULL )
    {
        ...
        *p++ = (unsigned char)( ( next_auth->id_type ) & 0xFF );
        ...
        if( next_auth->id != NULL && next_auth->len != 0 )
        {
...
#if defined(MBEDTLS_SSL_TRUSTED_CA_KEYS)
   if( ( ret = ssl_write_trusted_ca_keys_ext( ssl, p + 2 + ext_len,
                                              end, &olen ) ) != 0 )
   {
       MBEDTLS_SSL_DEBUG_RET( 1, "ssl_write_trusted_ca_keys_ext", ret );
       return( ret );
   }
   ext_len += olen;
#endif
