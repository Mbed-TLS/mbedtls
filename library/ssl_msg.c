...
static void ssl_extract_add_data_from_record(unsigned char* add_data, size_t* add_data_len, mbedtls_record* rec,
                                             unsigned minor_ver)
{
 ...
 */
static void ssl_build_record_nonce(unsigned char* dst_iv, size_t dst_iv_len, unsigned char const* fixed_iv,
                                   size_t fixed_iv_len, unsigned char const* dynamic_iv, size_t dynamic_iv_len)
{
        size_t i;
                            ...
                            int (*f_rng)(void*, unsigned char*, size_t), void* p_rng)
{
        ...
        if (rec->data_len > MBEDTLS_SSL_OUT_CONTENT_LEN) {
                MBEDTLS_SSL_DEBUG_MSG(1, ("Record content %" MBEDTLS_PRINTF_SIZET " too large, maximum "
        ...
        if (mode == MBEDTLS_MODE_STREAM || (mode == MBEDTLS_MODE_CBC
                ...
                if (transform->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_1) {
                        ...
                        ssl_extract_add_data_from_record(add_data, &add_data_len, rec, transform->minor_ver);
        ...
        if (mode == MBEDTLS_MODE_STREAM) {
                ...
                if ((ret = mbedtls_cipher_crypt(&transform->cipher_ctx_enc, transform->iv_enc, transform->ivlen, data,
                                                rec->data_len, data, &olen)) != 0) {
                ...
                ssl_build_record_nonce(iv, sizeof(iv), transform->iv_enc, transform->fixed_ivlen, dynamic_iv,
                                       dynamic_iv_len);
                ...
                if ((ret = mbedtls_cipher_auth_encrypt_ext(&transform->cipher_ctx_enc, iv, transform->ivlen, add_data,
                ...
                MBEDTLS_SSL_DEBUG_MSG(3, ("before encrypt: msglen = %" MBEDTLS_PRINTF_SIZET ", "
                                          "including %" MBEDTLS_PRINTF_SIZET " bytes of IV and %" MBEDTLS_PRINTF_SIZET
                                          " bytes of padding",
 ...
 */
MBEDTLS_STATIC_TESTABLE int mbedtls_ssl_cf_hmac(mbedtls_md_context_t* ctx, const unsigned char* add_data,
                                                size_t add_data_len, const unsigned char* data, size_t data_len_secret,
                                                size_t min_data_len, size_t max_data_len, unsigned char* output)
{
        /*
 ...
 */
MBEDTLS_STATIC_TESTABLE void mbedtls_ssl_cf_memcpy_offset(unsigned char* dst, const unsigned char* src_base,
                                                          size_t offset_secret, size_t offset_min, size_t offset_max,
                                                          size_t len)
{
        size_t offset;
...
int mbedtls_ssl_decrypt_buf(mbedtls_ssl_context const* ssl, mbedtls_ssl_transform* transform, mbedtls_record* rec)
{
        ...
        if (mode == MBEDTLS_MODE_STREAM) {
                padlen = 0;
                if ((ret = mbedtls_cipher_crypt(&transform->cipher_ctx_dec, transform->iv_dec, transform->ivlen, data,
                                                rec->data_len, data, &olen)) != 0) {
                        MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_crypt", ret);
                        return (ret);
                }
                 ...
                 */
                ssl_build_record_nonce(iv, sizeof(iv), transform->iv_dec, transform->fixed_ivlen, dynamic_iv,
                                       dynamic_iv_len);
                 ...
                 */
                if ((ret = mbedtls_cipher_auth_decrypt_ext(&transform->cipher_ctx_dec, iv, transform->ivlen, add_data,
                ...
                if (rec->data_len < minlen + transform->ivlen || rec->data_len < minlen + transform->maclen + 1) {
                        MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%" MBEDTLS_PRINTF_SIZET ") < max( "
                ...
                if (transform->encrypt_then_mac == MBEDTLS_SSL_ETM_ENABLED) {
                        ...
                        MBEDTLS_SSL_DEBUG_BUF(4, "MAC'd meta-data", add_data, add_data_len);
                        mbedtls_md_hmac_update(&transform->md_ctx_dec, add_data, add_data_len);
                        ...
                        if (mbedtls_ssl_safer_memcmp(data + rec->data_len, mac_expect, transform->maclen) != 0) {
                ...
                if (rec->data_len % transform->ivlen != 0) {
                        MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%" MBEDTLS_PRINTF_SIZET ") %% ivlen (%" MBEDTLS_PRINTF_SIZET
                                                  ") != 0",
                                                  rec->data_len, transform->ivlen));
                        return (MBEDTLS_ERR_SSL_INVALID_MAC);
                }
                ...
                if (auth_done == 1) {
                        const size_t mask = mbedtls_ssl_cf_mask_ge(rec->data_len, padlen + 1);
                        correct &= mask;
                        padlen &= mask;
                } else {
#if defined(MBEDTLS_SSL_DEBUG_ALL)
                        if (rec->data_len < transform->maclen + padlen + 1) {
                                MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%" MBEDTLS_PRINTF_SIZET ") < maclen "
                                                                                           "(%" MBEDTLS_PRINTF_SIZET
                        ...
                        const size_t mask = mbedtls_ssl_cf_mask_ge(rec->data_len, transform->maclen + padlen + 1);
                        correct &= mask;
                        padlen &= mask;
                }
        ...
        if (auth_done == 0) {
                ...
                ret = mbedtls_ssl_cf_hmac(&transform->md_ctx_dec, add_data, add_data_len, data, rec->data_len, min_len,
                                          max_len, mac_expect);
                if (ret != 0) {
                ...
                mbedtls_ssl_cf_memcpy_offset(mac_peer, data, rec->data_len, min_len, max_len, transform->maclen);
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
                ...
                if (mbedtls_ssl_safer_memcmp(mac_peer, mac_expect, transform->maclen) != 0) {
#if defined(MBEDTLS_SSL_DEBUG_ALL)
                        MBEDTLS_SSL_DEBUG_MSG(1, ("message mac does not match"));
#endif
                        correct = 0;
...
int mbedtls_ssl_fetch_input(mbedtls_ssl_context* ssl, size_t nb_want)
{
        ...
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
                ...
                MBEDTLS_SSL_DEBUG_MSG(2, ("in_left: %" MBEDTLS_PRINTF_SIZET ", nb_want: %" MBEDTLS_PRINTF_SIZET,
                                          ssl->in_left, nb_want));
...
#endif
        {
                ...
                while (ssl->in_left < nb_want) {
                        ...
                        if ((size_t)ret > len || (INT_MAX > SIZE_MAX && ret > (int)SIZE_MAX)) {
                                MBEDTLS_SSL_DEBUG_MSG(1, ("f_recv returned %d bytes but only %" MBEDTLS_PRINTF_SIZET
                                                          " were requested",
                                                          ret, len));
                                return (MBEDTLS_ERR_SSL_INTERNAL_ERROR);
                        }
...
int mbedtls_ssl_flush_output(mbedtls_ssl_context* ssl)
{
        ...
        while (ssl->out_left > 0) {
                ...
                if ((size_t)ret > ssl->out_left || (INT_MAX > SIZE_MAX && ret > (int)SIZE_MAX)) {
                        MBEDTLS_SSL_DEBUG_MSG(1, ("f_send returned %d bytes but only %" MBEDTLS_PRINTF_SIZET " bytes "
                                                                                                             "were "
                                                                                                             "sent",
                                                  ret, ssl->out_left));
                        return (MBEDTLS_ERR_SSL_INTERNAL_ERROR);
...
int mbedtls_ssl_write_record(mbedtls_ssl_context* ssl, uint8_t force_flush)
{
        ...
        if (!done) {
                ...
                if (ssl->transform_out != NULL) {
                        ...
                        mbedtls_ssl_write_version(ssl->major_ver, ssl->minor_ver, ssl->conf->transport, rec.ver);
                    ...
                    ((ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER && recv_msg_seq != ssl->handshake->in_msg_seq) ||
                     (ssl->state == MBEDTLS_SSL_HANDSHAKE_OVER && ssl->in_msg[0] != MBEDTLS_SSL_HS_CLIENT_HELLO))) {
                        if (recv_msg_seq > ssl->handshake->in_msg_seq) {
                                MBEDTLS_SSL_DEBUG_MSG(2, ("received future handshake message of sequence number %u "
                                                          "(next %u)",
                                        ...
                                        size_t in_len, unsigned char* obuf, size_t buf_len, size_t* olen)
{
         ...
         */
        if (in_len < 61 || in[0] != MBEDTLS_SSL_MSG_HANDSHAKE || in[3] != 0 || in[4] != 0 || in[19] != 0 ||
            in[20] != 0 || in[21] != 0) {
...
static int ssl_parse_record_header(mbedtls_ssl_context const* ssl, unsigned char* buf, size_t len, mbedtls_record* rec)
{
        ...
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM && ssl->conf->cid_len != 0 &&
            rec->type == MBEDTLS_SSL_MSG_CID) {
                ...
                if (len < rec_hdr_len_offset + rec_hdr_len_len) {
                        MBEDTLS_SSL_DEBUG_MSG(1, ("datagram of length %u too small to hold DTLS record header "
        ...
        mbedtls_ssl_read_version(&major_ver, &minor_ver, ssl->conf->transport, &rec->ver[0]);
        ...
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
                ...
                if (len < rec->data_offset + rec->data_len) {
                        MBEDTLS_SSL_DEBUG_MSG(1, ("Datagram of length %u too small to contain record of advertised "
                                                  "length %u.",
...
static int ssl_check_client_reconnect(mbedtls_ssl_context* ssl)
{
        ...
        if (rec_epoch == 0 && ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER &&
            ssl->state == MBEDTLS_SSL_HANDSHAKE_OVER && ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE &&
                ...
                if (!hs_buf->is_valid) {
                        ...
                        if (reassembly_buf_sz > (MBEDTLS_SSL_DTLS_MAX_BUFFERING - hs->buffering.total_bytes_buffered)) {
                                if (recv_msg_seq_offset > 0) {
                                        ...
                                        MBEDTLS_SSL_DEBUG_MSG(2, ("Buffering of future message of size "
                                                                  "%" MBEDTLS_PRINTF_SIZET " would exceed the "
                                                                                           "compile-time limit "
                                                                                           "%" MBEDTLS_PRINTF_SIZET
                                                                  " (already %" MBEDTLS_PRINTF_SIZET " bytes buffered) "
                                                                                                     "-- ignore\n",
                                ...
                                if (ssl_buffer_make_space(ssl, reassembly_buf_sz) != 0) {
                                        MBEDTLS_SSL_DEBUG_MSG(2, ("Reassembly of next message of size "
...
static int ssl_buffer_future_record(mbedtls_ssl_context* ssl, mbedtls_record const* rec)
{
        ...
        if (rec->buf_len > (MBEDTLS_SSL_DTLS_MAX_BUFFERING - hs->buffering.total_bytes_buffered)) {
                MBEDTLS_SSL_DEBUG_MSG(
                        2, ("Buffering of future epoch record of size %" MBEDTLS_PRINTF_SIZET " would exceed the "
                                                                                              "compile-time limit "
                                                                                              "%" MBEDTLS_PRINTF_SIZET
