...
hmac_failed_etm_disabled:
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_STREAM) {
        MBEDTLS_SSL_DEBUG_MSG(3,
                              ("before encrypt: msglen = %" MBEDTLS_PRINTF_SIZET
                               ", "
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_CBC ||
        ...
        MBEDTLS_SSL_DEBUG_MSG(3,
                              ("before encrypt: msglen = %" MBEDTLS_PRINTF_SIZET
                               ...
                               " bytes of IV and %" MBEDTLS_PRINTF_SIZET
                               " bytes of padding",
        ...
        status = psa_cipher_set_iv(&cipher_op, transform->iv_enc,
                                   transform->ivlen);
                            ...
                            mbedtls_record            *rec)
{
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_AEAD) {
        ...
        if ((ret = mbedtls_cipher_auth_decrypt_ext(&transform->cipher_ctx_dec,
                                                   ...
                                                   data,
                                                   rec->data_len +
                                                   transform->taglen, /* src */
                                                   data,
                                                   rec->buf_len -
                                                   (data - rec->buf), &olen, /* dst */
                                                   transform->taglen)) != 0) {
...
hmac_failed_etm_enabled:
        ...
        status = psa_cipher_set_iv(&cipher_op, transform->iv_dec,
                                   transform->ivlen);
...
#endif
    {
        ...
        while (ssl->in_left < nb_want) {
            ...
            if ((size_t)ret > len ||
                ...
                MBEDTLS_SSL_DEBUG_MSG(1,
                                      ("f_recv returned %d bytes but only %"
                                       MBEDTLS_PRINTF_SIZET " were requested",
                                       ret, len));
                return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
            }
...
int mbedtls_ssl_flush_output(mbedtls_ssl_context *ssl)
{
    ...
    while (ssl->out_left > 0) {
        ...
        if ((size_t)ret > ssl->out_left ||
            (INT_MAX > SIZE_MAX && ret > (int)SIZE_MAX)) {
            MBEDTLS_SSL_DEBUG_MSG(1,
                                  ("f_send returned %d bytes but only %"
                                   MBEDTLS_PRINTF_SIZET " bytes were sent",
                                   ret, ssl->out_left));
            return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
        }
    ...
    if (ssl->out_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE) {
        ...
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
            ...
            if (hs_type != MBEDTLS_SSL_HS_HELLO_REQUEST) {
                MBEDTLS_PUT_UINT16_BE(ssl->handshake->out_msg_seq, ssl->out_msg,
                                      4);
