...
read_record_header:
    ...
    ret = mbedtls_ssl_confirm_content_len( ssl, 1, MBEDTLS_SSL_BUFFER_MIN);
     if( ret )
         return( ret );
...
static int ssl_write_hello_verify_request( mbedtls_ssl_context *ssl )
{
    ...
    if( ( ret = ssl->conf->f_cookie_write( ssl->conf->p_cookie,
                                     &p, ssl->out_buf + ssl->out_max_content_len + MBEDTLS_SSL_PAYLOAD_OVERHEAD,
                                     ssl->cli_id, ssl->cli_id_len ) ) != 0 )
    {
...
curve_matching_done:
    ...
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
        ...
        if( ssl->conf->f_async_sign_start != NULL )
        {
         ...
         len = ssl->out_msg + ssl->out_msglen - ssl->out_buf;
         ret = mbedtls_ssl_confirm_content_len( ssl, 1,
                                                ( ssl->out_msg + ssl->out_msglen + 4 - ssl->out_buf ) +
                                                mbedtls_pk_get_len(
						     mbedtls_ssl_own_key( ssl ) ) );
         if( ret )
             return( ret );

        ssl->out_msglen  = ssl->out_buf + len - ssl->out_msg;
