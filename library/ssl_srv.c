...
read_record_header:
    ...
    {
        if( p[0] == MBEDTLS_CHAR_1( MBEDTLS_SSL_FALLBACK_SCSV_VALUE ) &&
            p[1] == MBEDTLS_CHAR_0( MBEDTLS_SSL_FALLBACK_SCSV_VALUE ) )
        {
...
#endif
        {
            if( p[0] != MBEDTLS_CHAR_1( ciphersuites[i] ) ||
                p[1] != MBEDTLS_CHAR_0( ciphersuites[i] ) )
                continue;
