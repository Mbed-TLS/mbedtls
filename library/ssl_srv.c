...
static int ssl_parse_use_srtp_ext(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len)
{
        ...
        if (profile_length > len - size_of_lengths || profile_length % 2 != 0) /* profiles are 2 bytes long, so the
                                                                                  length must be even */
        {
                mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL, MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR);
                return (MBEDTLS_ERR_SSL_DECODE_ERROR);
...
read_record_header:
        ...
        MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, message len.: %d", (ssl->in_len[0] << 8) | ssl->in_len[1]));
        ...
        if (sess_len > sizeof(ssl->session_negotiate->id) || sess_len + 34 + 2 > msg_len) /* 2 for cipherlist length
                                                                                             field */
        {
                MBEDTLS_SSL_DEBUG_MSG(1, ("bad client hello message"));
                mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL, MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR);
...
static void ssl_handle_id_based_session_resumption(mbedtls_ssl_context* ssl)
{
        ...
        if (session->ciphersuite != session_tmp.ciphersuite || session->compression != session_tmp.compression) {
...
static int ssl_prepare_server_key_exchange(mbedtls_ssl_context* ssl, size_t* signature_len)
{
        ...
        if (mbedtls_ssl_ciphersuite_uses_dhe(ciphersuite_info)) {
                 ...
                 * } ServerDHParams;
                 */
                if ((ret = mbedtls_dhm_set_group(&ssl->handshake->dhm_ctx, &ssl->conf->dhm_P, &ssl->conf->dhm_G)) !=
                    0) {
                ...
                if ((ret = mbedtls_dhm_make_params(
                             &ssl->handshake->dhm_ctx, (int)mbedtls_dhm_get_len(&ssl->handshake->dhm_ctx),
        ...
        if (mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info)) {
                ...
                if (md_alg != MBEDTLS_MD_NONE) {
                        ret = mbedtls_ssl_get_key_exchange_md_tls1_2(ssl, hash, &hashlen, dig_signed, dig_signed_len,
                                                                     md_alg);
                        if (ret != 0)
                                return (ret);
                } else
                                   ...
                                   size_t pms_offset)
{
        ...
        mbedtls_ssl_write_version(ssl->handshake->max_major_ver, ssl->handshake->max_minor_ver, ssl->conf->transport,
                                  ver);
...
static int ssl_parse_certificate_verify(mbedtls_ssl_context* ssl)
{
        ...
        if ((ret = mbedtls_pk_verify(peer_pk, md_alg, hash_start, hashlen, ssl->in_msg + i, sig_len)) != 0) {
