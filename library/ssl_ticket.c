 ...
 */
int mbedtls_ssl_ticket_setup(mbedtls_ssl_ticket_context* ctx, int (*f_rng)(void*, unsigned char*, size_t), void* p_rng,
                             mbedtls_cipher_type_t cipher, uint32_t lifetime)
{
        int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
        ...
        ret = mbedtls_cipher_setup_psa(&ctx->keys[0].ctx, cipher_info, TICKET_AUTH_TAG_BYTES);
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
        ret = mbedtls_cipher_setup_psa(&ctx->keys[1].ctx, cipher_info, TICKET_AUTH_TAG_BYTES);
        if (ret != 0 && ret != MBEDTLS_ERR_CIPHER_FEATURE_UNAVAILABLE)
                return (ret);
        if (ret == MBEDTLS_ERR_CIPHER_FEATURE_UNAVAILABLE)
#endif /* MBEDTLS_USE_PSA_CRYPTO */
...
int mbedtls_ssl_ticket_write(void* p_ticket, const mbedtls_ssl_session* session, unsigned char* start,
                             const unsigned char* end, size_t* tlen, uint32_t* ticket_lifetime)
{
        int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
        ...
        *ticket_lifetime = ctx->ticket_lifetime;
        ...
        /* Dump session state */
        if ((ret = mbedtls_ssl_session_save(session, state, end - state, &clear_len)) != 0 ||
        ...
        if ((ret = mbedtls_cipher_auth_encrypt_ext(&key->ctx, iv, TICKET_IV_BYTES,
                                                   ...
                                                   key_name, TICKET_ADD_DATA_LEN, state, clear_len, state, end - state,
