                        ...
                        size_t              own_cid_len)
{
    ...
    if (own_cid_len != ssl->conf->cid_len) {
        MBEDTLS_SSL_DEBUG_MSG(3,
                              (
                                  "CID length %u does not match CID length %u in config",
                             ...
                             const mbedtls_ssl_session *src)
{
    ...
    if (src->peer_cert != NULL) {
        ...
        if ((ret =
                 mbedtls_x509_crt_parse_der(dst->peer_cert,
                                            src->peer_cert->raw.p,
                                   ...
                                   size_t out_buf_new_len)
{
    ...
    if (ssl->in_buf != NULL) {
        ...
        if (downsizing ?
            ...
            if (resize_buffer(&ssl->in_buf, in_buf_new_len,
                              &ssl->in_buf_len) != 0) {
                MBEDTLS_SSL_DEBUG_MSG(1,
                                      (
                                          "input buffer resizing failed - out of memory"));
            } else {
    ...
    if (ssl->out_buf != NULL) {
        ...
        if (downsizing ?
            ...
            if (resize_buffer(&ssl->out_buf, out_buf_new_len,
                ...
                MBEDTLS_SSL_DEBUG_MSG(1,
                                      (
                                          "output buffer resizing failed - out of memory"));
            } else {
                MBEDTLS_SSL_DEBUG_MSG(2,
                                      ("Reallocating out_buf to %"
                                       MBEDTLS_PRINTF_SIZET,
                                       out_buf_new_len));
                modified = 1;
            }
...
static void ssl_calc_verify_tls_sha256(const mbedtls_ssl_context *,
                                       unsigned char *, size_t *);
static void ssl_calc_finished_tls_sha256(mbedtls_ssl_context *, unsigned char *,
                                         int);
...
static void ssl_calc_verify_tls_sha384(const mbedtls_ssl_context *,
                                       unsigned char *, size_t *);
static void ssl_calc_finished_tls_sha384(mbedtls_ssl_context *, unsigned char *,
                                         int);
...
static void ssl_update_checksum_start(mbedtls_ssl_context *,
                                      const unsigned char *, size_t);
...
static int ssl_conf_version_check(const mbedtls_ssl_context *ssl)
{
    ...
    if (mbedtls_ssl_conf_is_hybrid_tls12_tls13(conf)) {
        ...
        MBEDTLS_SSL_DEBUG_MSG(4,
                              ("The SSL configuration is TLS 1.3 or TLS 1.2."));
        return 0;
    }
#endif
    ...
    size_t                *key_size)
{
    switch (mbedtls_cipher_type) {
        ...
        case MBEDTLS_CIPHER_AES_128_CCM:
            *alg =
                taglen ? PSA_ALG_AEAD_WITH_SHORTENED_TAG(PSA_ALG_CCM,
                                                         taglen) : PSA_ALG_CCM;
            *key_type = PSA_KEY_TYPE_AES;
            *key_size = 128;
            break;
...
#endif /* MBEDTLS_SSL_SOME_SUITES_USE_CBC_ETM */
                                       ssl_tls12prf_from_cs(ssl->session->
                                                            ciphersuite),
                                ...
                                int endpoint, int transport, int preset)
{
    ...
    if (ssl_check_no_sig_alg_duplication(ssl_tls12_preset_suiteb_sig_algs)) {
        mbedtls_printf(
            "ssl_tls12_preset_suiteb_sig_algs has duplicated entries\n");
        return MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    }

    if (ssl_check_no_sig_alg_duplication(ssl_tls12_preset_default_sig_algs)) {
        mbedtls_printf(
            "ssl_tls12_preset_default_sig_algs has duplicated entries\n");
        return MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    }
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
#endif /* MBEDTLS_DEBUG_C && MBEDTLS_KEY_EXCHANGE_WITH_CERT_ENABLED */
 ...
 */
int mbedtls_ssl_check_curve_tls_id(const mbedtls_ssl_context *ssl,
                                   uint16_t                  tls_id)
{
...
static psa_status_t setup_psa_key_derivation(
    ...
    psa_algorithm_t                alg,
    const unsigned char            *raw_psk,
    size_t                         raw_psk_length,
                              ...
                              const mbedtls_ssl_context    *ssl)
{
    ...
    if (mbedtls_ssl_ciphersuite_uses_psk(handshake->ciphersuite_info) == 1) {
        ...
        switch (handshake->ciphersuite_info->key_exchange) {
            ...
            case MBEDTLS_KEY_EXCHANGE_DHE_PSK:
                other_secret_len =
                    MBEDTLS_GET_UINT16_BE(handshake->premaster, 0);
                other_secret = handshake->premaster + 2;
                break;
            default:
                break;
...
int mbedtls_ssl_resend_hello_request(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->conf->renego_max_records < 0) {
        uint32_t ratio = ssl->conf->hs_timeout_max / ssl->conf->hs_timeout_min +
                         1;
                                        ...
                                        void                *rs_ctx)
{
    ...
    if (ssl->session_negotiate->verify_result != 0) {
        MBEDTLS_SSL_DEBUG_MSG(3, ("! Certificate verification flags %08x",
                                  (unsigned int)ssl->session_negotiate->
                                  verify_result));
    } else {
                                        ...
                                        const mbedtls_ssl_context    *ssl)
{
    ...
    if (mac_alg == 0) {
        MBEDTLS_SSL_DEBUG_MSG(1,
                              ("mbedtls_hash_info_psa_from_md for %u not found",
...
#else
        if (mbedtls_cipher_info_get_mode(cipher_info) ==
    ...
    if ((ret = mbedtls_cipher_setkey(&transform->cipher_ctx_enc, key1,
                                     (int)mbedtls_cipher_info_get_key_bitlen(
                                         cipher_info),
                                     MBEDTLS_ENCRYPT)) != 0) {
...
#else
        ret = mbedtls_md_hmac_starts(&transform->md_ctx_enc, mac_enc,
                                     mac_key_len);
        if (ret != 0) {
            goto end;
        }
        ret = mbedtls_md_hmac_starts(&transform->md_ctx_dec, mac_dec,
                                     mac_key_len);
        if (ret != 0) {
            goto end;
        }
                                           ...
                                           mbedtls_md_type_t md_alg)
{
    ...
    MBEDTLS_SSL_DEBUG_MSG(3,
                          (
                              "Perform PSA-based computation of digest of ServerKeyExchange"));
