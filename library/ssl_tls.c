 ...
 */
static unsigned int mfl_code_to_length(int mfl, unsigned int max_content_len)
{
    switch(mfl) {
...
static int ssl_decompress_buf( mbedtls_ssl_context *ssl )
{
    ...
    ssl->transform_in->ctx_inflate.avail_out = MBEDTLS_SSL_IN_CONTENT_LEN;
...
{
    int ret;
    const size_t in_len = MBEDTLS_SSL_IN_BUFFER_LEN;
    const size_t out_len = MBEDTLS_SSL_OUT_BUFFER_LEN;
     ...
     */
    if( ( ssl->in_buf = mbedtls_calloc( 1, in_len ) ) == NULL ||
        ( ssl->out_buf = mbedtls_calloc( 1, out_len ) ) == NULL )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "alloc(%d bytes) failed", ssl->in_buf ? out_len : in_len) );
...
int mbedtls_ssl_conf_max_frag_len( mbedtls_ssl_config *conf, unsigned char mfl_code )
{
    if( mfl_code >= MBEDTLS_SSL_MAX_FRAG_LEN_INVALID ||
        mfl_code_to_length( mfl_code, MBEDTLS_TLS_EXT_ADV_CONTENT_LEN) > MBEDTLS_TLS_EXT_ADV_CONTENT_LEN )
    {
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    }
...
size_t mbedtls_ssl_get_max_frag_len( const mbedtls_ssl_context *ssl )
{
     ...
     */
    max_len = mfl_code_to_length( ssl->conf->mfl_code, MBEDTLS_TLS_EXT_ADV_CONTENT_LEN );

    /*
     * Check if a smaller max length was negotiated
     */
    if( ssl->session_out != NULL &&
        mfl_code_to_length( ssl->session_out->mfl_code, MBEDTLS_TLS_EXT_ADV_CONTENT_LEN) < max_len )
    {
        max_len = mfl_code_to_length( ssl->session_out->mfl_code, MBEDTLS_TLS_EXT_ADV_CONTENT_LEN );
    }
