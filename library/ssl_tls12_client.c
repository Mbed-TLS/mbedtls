                                        ...
                                        size_t *olen)
{
    ...
    *olen = 4;
    ...
    MBEDTLS_SSL_DEBUG_MSG(3,
                          ("sending session ticket of length %"
                           MBEDTLS_PRINTF_SIZET, tlen));
    ...
    *p++ = MBEDTLS_BYTE_0(2 * ssl->conf->dtls_srtp_profile_list_len);
    ...
    for (protection_profiles_index = 0;
        ...
        profile_value = mbedtls_ssl_check_srtp_profile_value
                            (ssl->conf->dtls_srtp_profile_list[
                                protection_profiles_index]);
                                  ...
                                  size_t len)
{
    ...
    if (len > 5  && (buf[4] != mki_len ||
                     (memcmp(ssl->dtls_srtp_info.mki_value, &buf[5],
                             mki_len)))) {
        mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                       MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER);
        return MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER;
        ...
        switch (ext_id) {
          ...
          MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED */
            ...
            case MBEDTLS_TLS_EXT_USE_SRTP:
                ...
                if ((ret =
                         ssl_parse_use_srtp_ext(ssl, ext + 4, ext_size)) != 0) {
                    return ret;
                }
                                      ...
                                      unsigned char *end)
{
    ...
    if (dhm_actual_bitlen < ssl->conf->dhm_min_bitlen) {
        MBEDTLS_SSL_DEBUG_MSG(1,
                              ("DHM prime too short: %" MBEDTLS_PRINTF_SIZET
                               " < %u",
                               dhm_actual_bitlen,
...
static int ssl_write_client_key_exchange(mbedtls_ssl_context *ssl)
{
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_DHE_RSA) {
        ...
        ret = mbedtls_dhm_make_public(&ssl->handshake->dhm_ctx,
                                      (int)mbedtls_dhm_get_len(&ssl->handshake->
                                                               dhm_ctx),
...
static int ssl_parse_new_session_ticket(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_DEBUG_MSG(3,
                          ("ticket length: %" MBEDTLS_PRINTF_SIZET,
                           ticket_len));
