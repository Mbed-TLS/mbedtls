                                  ...
                                  size_t len)
{
    ...
    for (j = 0; j < profile_length; j += 2) {
        ...
        client_protection = mbedtls_ssl_check_srtp_profile_value(
            protection_profile_value);
                         ...
                         const mbedtls_ssl_ciphersuite_t *ciphersuite_info)
{
    ...
    for (cur = list; cur != NULL; cur = cur->next) {
        ...
        key_type_matches = ((ssl->conf->f_async_sign_start != NULL ||
                             ...
                             mbedtls_pk_can_do_ext(cur->key, pk_alg,
                                                   pk_usage)) &&
                            mbedtls_pk_can_do_ext(&cur->cert->pk, pk_alg,
                                                  pk_usage));
#else
        key_type_matches = (
    ...
    while (ext_len != 0) {
        ...
        switch (ext_id) {
            ...
            case MBEDTLS_TLS_EXT_SIG_ALG:
                ...
                ret = mbedtls_ssl_parse_sig_alg_ext(ssl,
                                                    ext + 4,
                                                    ext + 4 + ext_size);
                if (ret != 0) {
                    return ret;
...
have_ciphersuite:
    ...
    if (sig_alg != MBEDTLS_PK_NONE) {
        ...
        MBEDTLS_SSL_DEBUG_MSG(3,
                              ("client hello v3, signature_algorithm ext: %u",
                               sig_hash));
    } else {
...
#endif \
    /* MBEDTLS_ECDH_C || MBEDTLS_ECDSA_C || MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED */
...
static int ssl_write_server_hello(mbedtls_ssl_context *ssl)
{
    ...
    *p++ = MBEDTLS_BYTE_0(MBEDTLS_SSL_COMPRESS_NULL);
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, chosen ciphersuite: %s",
                              mbedtls_ssl_get_ciphersuite_name(ssl->
                                                               session_negotiate
                                                               ->ciphersuite)));
...
static int ssl_write_certificate_request(mbedtls_ssl_context *ssl)
{
    ...
    unsigned char *buf, *p;
    const unsigned char * const end = ssl->out_msg +
    ...
    switch (mbedtls_pk_get_type(pk)) {
        ...
        case MBEDTLS_PK_ECDSA:
            ...
            psa_set_key_type(&key_attributes,
                             PSA_KEY_TYPE_ECC_KEY_PAIR(ssl->handshake->
                                                       ecdh_psa_type));
...
static int ssl_parse_client_key_exchange(mbedtls_ssl_context *ssl)
{
    ...
    if ((ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_RSA_PSK ||
        ...
        MBEDTLS_SSL_DEBUG_MSG(3,
                              (
                                  "will resume decryption of previously-read record"));
    } else
#endif
