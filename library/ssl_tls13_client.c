...
static int ssl_tls13_write_named_group_ecdhe( mbedtls_ssl_context *ssl,
                                              ...
                                              size_t *olen )
{
    unsigned char *named_group_iter = buf; /* Iteration over named_group_list */
    ...
    *olen = 0;
          ...
          info++ )
    {
        ...
        MBEDTLS_SSL_CHK_BUF_PTR( named_group_iter, end, 2);
        MBEDTLS_PUT_UINT16_BE( info->tls_id, named_group_iter, 0 );
        named_group_iter += 2;
...
static int ssl_tls13_write_supported_groups_ext( mbedtls_ssl_context *ssl,
                                                 ...
                                                 size_t *olen )
{
    unsigned char *named_group_start = buf + 6; /* Start of named_group_list */
    unsigned char *named_group_iter = buf + 6;  /* Iteration over named_group_list */
    ...
    *olen = 0;
    ...
    ret_ecdhe = ssl_tls13_write_named_group_ecdhe( ssl, named_group_iter, end, &named_group_len );
    if( ret_ecdhe != 0 )
    {
        MBEDTLS_SSL_DEBUG_RET( 1, "ssl_tls13_write_named_group_ecdhe", ret );
    }
    named_group_iter += named_group_len;

    ret_dhe = ssl_tls13_write_named_group_dhe( ssl, named_group_iter, end, &named_group_len );
    if( ret_dhe != 0 )
    {
    ...
    named_group_len = named_group_iter - named_group_start;
                                              ...
                                              size_t *olen )
{
    ...
    *buf++ = MBEDTLS_SSL_COMPRESS_NULL;
     ...
     *    from above )
     */
    ret = ssl_tls13_write_key_share_ext( ssl, buf, end, &output_len );
    if( ret != 0 )
        return( ret );
    buf += output_len;
