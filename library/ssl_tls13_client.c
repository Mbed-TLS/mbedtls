                                         ...
                                         size_t *out_len)
{
    ...
    *out_len = p - buf;
    ...
    MBEDTLS_SSL_DEBUG_BUF(3, "client hello, key_share extension", buf,
                          *out_len);
                                   ...
                                   const unsigned char *end)
{
     ...
     *
     */
    MBEDTLS_SSL_CHK_BUF_READ_PTR(buf,
                                 end,
                                 2 +
                                 sizeof(
                                     mbedtls_ssl_tls13_hello_retry_request_magic));
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_check_server_hello_session_id_echo(
    mbedtls_ssl_context *ssl,
    const unsigned char **buf,
    const unsigned char *end)
{
    ...
    if (ssl->session_negotiate->id_len != legacy_session_id_echo_len ||
        memcmp(ssl->session_negotiate->id, p,
...
static int ssl_tls13_postprocess_server_hello(mbedtls_ssl_context *ssl)
{
    ...
    if (!mbedtls_ssl_conf_tls13_check_kex_modes(ssl,
                                                handshake->key_exchange_mode)) {
        ret = MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
        MBEDTLS_SSL_DEBUG_MSG(2,
                              ("Key exchange mode(%s) is not supported.",
                               ssl_tls13_get_kex_mode_str(handshake->
                                                          key_exchange_mode)));
        goto cleanup;
    }
    ...
    while (p < extensions_end) {
        ...
        switch (extension_type) {
            ...
            case MBEDTLS_TLS_EXT_ALPN:
                ...
                if ((ret =
                         ssl_tls13_parse_alpn_ext(ssl, p,
                                                  (size_t)extension_data_len))
                    != 0) {
                    return ret;
                }
...
static int ssl_tls13_process_certificate_request(mbedtls_ssl_context *ssl)
{
    ...
    if (ret == SSL_CERTIFICATE_REQUEST_EXPECT_REQUEST) {
        ...
        mbedtls_ssl_add_hs_msg_to_checksum(ssl,
                                           MBEDTLS_SSL_HS_CERTIFICATE_REQUEST,
                                           buf,
                                           buf_len);
