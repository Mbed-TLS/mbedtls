...
int mbedtls_ssl_tls13_write_sig_alg_ext( mbedtls_ssl_context *ssl,
                                         ...
                                         size_t *olen )
{
    unsigned char *sig_alg_iter = buf + 6;  /* Iteration over supported_signature_algorithms */
    unsigned char *sig_alg_start = buf + 6; /* Start of supported_signature_algorithms */
    ...
    *olen = 0;
    ...
    {
        MBEDTLS_SSL_CHK_BUF_PTR( sig_alg_iter, end, 2 );
        MBEDTLS_PUT_UINT16_BE( *sig_alg, sig_alg_iter, 0 );
        sig_alg_iter += 2;
        MBEDTLS_SSL_DEBUG_MSG( 3, ( "signature scheme [%x]", *sig_alg ) );
    }

    /* Length of supported_signature_algorithms*/
    sig_alg_len = sig_alg_iter - sig_alg_start;
    if( sig_alg_len == 0 )
    {
