 ...
 */
static void ssl_tls13_create_verify_structure(
                                              ...
                                              size_t verify_buffer_len)
{
    ...
    MBEDTLS_SSL_DEBUG_MSG(3,
                          ("Certificate Verify: Signature algorithm ( %04x )",
                           (unsigned int)algorithm));
    ...
    if (!mbedtls_pk_can_do(&ssl->session_negotiate->peer_cert->pk, sig_alg)) {
        MBEDTLS_SSL_DEBUG_MSG(1,
                              ("signature algorithm doesn't match cert key"));
        goto error;
    }
...
cleanup:
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("<= parse certificate verify"));
    MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_ssl_tls13_process_certificate_verify",
                          ret);
    return ret;
#else
    ((void)ssl);
...
static int ssl_tls13_validate_certificate(mbedtls_ssl_context *ssl)
{
    ...
    if (ret != 0) {
        ...
        if (verify_result & MBEDTLS_X509_BADCERT_OTHER) {
            MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_ACCESS_DENIED,
                                         ret);
            ...
            MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_CERT_EXPIRED,
                                         ret);
        } else if (verify_result & MBEDTLS_X509_BADCERT_REVOKED) {
            MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_CERT_REVOKED,
                                         ret);
        } else if (verify_result & MBEDTLS_X509_BADCERT_NOT_TRUSTED) {
            MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_UNKNOWN_CA, ret);
        } else {
            MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_CERT_UNKNOWN,
                                         ret);
        }
    }
                                                   ...
                                                   size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    for (; *sig_alg != MBEDTLS_TLS1_3_SIG_NONE; sig_alg++) {
        ...
        if (!mbedtls_ssl_tls13_check_sig_alg_cert_key_match(*sig_alg,
                                                            own_key)) {
            continue;
        }
        ...
        if ((ret = mbedtls_pk_sign_ext(pk_type, own_key,
            ...
            MBEDTLS_SSL_DEBUG_MSG(2,
                                  ("CertificateVerify signature failed with %s",
...
static int ssl_tls13_preprocess_finished_message(mbedtls_ssl_context *ssl)
{
    ...
    ret = mbedtls_ssl_tls13_calculate_verify_data(ssl,
                                                  ...
                                                  sizeof(ssl->handshake->
                                                         state_local.finished_in
                                                         .digest),
                                                  &ssl->handshake->state_local.finished_in.digest_len,
                                                  ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT ?
                                                  MBEDTLS_SSL_IS_SERVER : MBEDTLS_SSL_IS_CLIENT);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_ssl_tls13_calculate_verify_data",
                              ret);
        return ret;
    }
...
int mbedtls_ssl_tls13_process_finished_message(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_PROC_CHK(ssl_tls13_parse_finished_message(ssl, buf,
                                                          buf + buf_len));
