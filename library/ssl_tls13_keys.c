    ...
    unsigned char *buf, size_t buf_len)
{
    ...
    status =
        psa_key_derivation_setup(&operation, PSA_ALG_HKDF_EXPAND(hash_alg));
                                            ...
                                            int                 from)
{
    ...
    ret = ssl_tls13_calc_finished_core(hash_alg, base_key, transcript, dst,
                                       actual_len);
    if (ret != 0) {
        goto exit;
    }
                                         ...
                                         mbedtls_ssl_context       *ssl /* DEBUG ONLY */)
{
    ...
    if ((status = mbedtls_ssl_cipher_to_psa(ciphersuite_info->cipher,
        ...
        MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_ssl_cipher_to_psa", psa_ssl_status_to_mbedtls(
                                  status));
        return psa_ssl_status_to_mbedtls(status);
    }
    ...
    if (alg != MBEDTLS_SSL_NULL_CIPHER) {
        ...
        if ((status = psa_import_key(&attributes,
            ...
            MBEDTLS_SSL_DEBUG_RET(1, "psa_import_key", psa_ssl_status_to_mbedtls(
                                      status));
            return psa_ssl_status_to_mbedtls(status);
        }
...
int mbedtls_ssl_tls13_key_schedule_stage_handshake(mbedtls_ssl_context *ssl)
{
    ...
    if (mbedtls_ssl_tls13_key_exchange_mode_with_ephemeral(ssl)) {
        ...
        } else if (mbedtls_ssl_tls13_named_group_is_dhe(handshake->
                                                        offered_group_id)) {
...
int mbedtls_ssl_tls13_compute_application_transform(mbedtls_ssl_context *ssl)
{
    ...
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1,
                              "mbedtls_ssl_tls13_generate_application_keys",
                              ret);
        goto cleanup;
    }
                                           ...
                                           size_t              *psk_len)
{
    ...
    *psk = NULL;
    ...
    status =
        psa_get_key_attributes(ssl->handshake->psk_opaque, &key_attributes);
    if (status != PSA_SUCCESS) {
        return psa_ssl_status_to_mbedtls(status);
    }
