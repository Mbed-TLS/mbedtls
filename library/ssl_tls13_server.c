    ...
    const mbedtls_ssl_ciphersuite_t **selected_ciphersuite_info)
{
    *selected_ciphersuite = 0;
    *selected_ciphersuite_info = NULL;
    for (const unsigned char *p = cipher_suites; p < cipher_suites_end;
         p += 2) {
                                              ...
                                              const unsigned char *ciphersuites_end)
{
    ...
    MBEDTLS_SSL_CHK_BUF_READ_PTR(p_binder_len, pre_shared_key_ext_end,
                                 binders_len);
    ...
    while (p_identity_len < identities_end && p_binder_len < binders_end) {
        ...
        MBEDTLS_SSL_CHK_BUF_READ_PTR(identity, identities_end,
                                     identity_len + 4);
...
static int ssl_tls13_postprocess_client_hello(mbedtls_ssl_context *ssl)
{
    ...
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1,
                              "mbedtls_ssl_tls1_3_key_schedule_stage_early",
                              ret);
        return ret;
    }
                                             ...
                                             int                 is_hrr)
{
    ...
    *p++ = MBEDTLS_SSL_COMPRESS_NULL;
    ...
    if (!is_hrr && mbedtls_ssl_tls13_key_exchange_mode_with_psk(ssl)) {
        ret =
            ssl_tls13_write_server_pre_shared_key_ext(ssl, p, end, &output_len);
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_write_new_session_ticket_coordinate(
    mbedtls_ssl_context *ssl)
{
...
int mbedtls_ssl_tls13_handshake_server_step(mbedtls_ssl_context *ssl)
{
    ...
    switch (ssl->state) {
        ...
        case MBEDTLS_SSL_HELLO_RETRY_REQUEST:
            ...
            if (ret != 0) {
                MBEDTLS_SSL_DEBUG_RET(1, "ssl_tls13_write_hello_retry_request",
                                      ret);
                return ret;
            }
            break;
        ...
        case MBEDTLS_SSL_ENCRYPTED_EXTENSIONS:
            ...
            if (ret != 0) {
                MBEDTLS_SSL_DEBUG_RET(1, "ssl_tls13_write_encrypted_extensions",
                                      ret);
                return ret;
            }
            break;
