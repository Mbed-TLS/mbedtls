...
static int x509_get_entries(unsigned char** p, const unsigned char* end, mbedtls_x509_crl_entry* entry)
{
        ...
        while (*p < end) {
                ...
                cur_entry->raw.tag = **p;
                if ((ret = mbedtls_asn1_get_tag(p, end, &len2, MBEDTLS_ASN1_SEQUENCE | MBEDTLS_ASN1_CONSTRUCTED)) !=
                    0) {
                        return (ret);
                }
...
int mbedtls_x509_crl_parse_der(mbedtls_x509_crl* chain, const unsigned char* buf, size_t buflen)
{
        ...
        if ((ret = mbedtls_x509_get_sig_alg(&crl->sig_oid, &sig_params1, &crl->sig_md, &crl->sig_pk, &crl->sig_opts)) !=
            0) {
                mbedtls_x509_crl_free(crl);
        ...
        if ((ret = mbedtls_x509_get_time(&p, end, &crl->next_update)) != 0) {
                if (ret != (MBEDTLS_ERROR_ADD(MBEDTLS_ERR_X509_INVALID_DATE, MBEDTLS_ERR_ASN1_UNEXPECTED_TAG)) &&
                    ret != (MBEDTLS_ERROR_ADD(MBEDTLS_ERR_X509_INVALID_DATE, MBEDTLS_ERR_ASN1_OUT_OF_DATA))) {
                        mbedtls_x509_crl_free(crl);
                        return (ret);
                }
        }
        ...
        if (crl->sig_oid.len != sig_oid2.len || memcmp(crl->sig_oid.p, sig_oid2.p, crl->sig_oid.len) != 0 ||
...
int mbedtls_x509_crl_parse(mbedtls_x509_crl* chain, const unsigned char* buf, size_t buflen)
{
        ...
        do {
                ...
                if (buflen == 0 || buf[buflen - 1] != '\0')
                        ...
                        ret = mbedtls_pem_read_buffer(&pem, "-----BEGIN X509 CRL-----", "-----END X509 CRL-----", buf,
                                                      NULL, 0, &use_len);
