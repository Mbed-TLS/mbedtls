                                      ...
                                      int *max_pathlen)
{
    ...
    *max_pathlen = 0; /* endless */
    ...
    if ((ret = mbedtls_asn1_get_bool(p, end, ca_istrue)) != 0) {
        ...
        if (ret != 0)
            return (
                MBEDTLS_ERROR_ADD(MBEDTLS_ERR_X509_INVALID_EXTENSIONS, ret));
...
{
    return (
        mbedtls_x509_crt_parse_der_internal(chain, buf, buflen, 0, NULL, NULL));
}
...
{
    return (
        mbedtls_x509_crt_parse_der_internal(chain, buf, buflen, 1, NULL, NULL));
}
