...
int mbedtls_x509write_crt_set_key_usage(mbedtls_x509write_cert* ctx, unsigned int key_usage)
{
        ...
        ret = mbedtls_x509write_crt_set_extension(ctx, MBEDTLS_OID_KEY_USAGE, MBEDTLS_OID_SIZE(MBEDTLS_OID_KEY_USAGE),
                                                  1, c, (size_t)ret);
        if (ret != 0)
                return (ret);
                              ...
                              int (*f_rng)(void*, unsigned char*, size_t), void* p_rng)
{
        ...
        if (ctx->version == MBEDTLS_X509_CRT_VERSION_3) {
                ...
                MBEDTLS_ASN1_CHK_ADD(
                        len,
                        mbedtls_asn1_write_tag(&c, buf, MBEDTLS_ASN1_CONTEXT_SPECIFIC | MBEDTLS_ASN1_CONSTRUCTED | 3));
        }
        ...
        if ((ret = mbedtls_pk_sign(ctx->issuer_key, ctx->md_alg, hash, 0, sig, sizeof(sig), &sig_len, f_rng, p_rng)) !=
            0) {
                return (ret);
        }
                              ...
                              int (*f_rng)(void*, unsigned char*, size_t), void* p_rng)
{
        ...
        if ((ret = mbedtls_pem_write_buffer(PEM_BEGIN_CRT, PEM_END_CRT, buf + size - ret, ret, buf, size, &olen)) !=
            0) {
                return (ret);
        }
