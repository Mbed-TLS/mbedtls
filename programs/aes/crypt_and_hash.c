...
int main(int argc, char *argv[])
{
    ...
    if (mode == MODE_ENCRYPT) {
        ...
        if (mbedtls_md_hmac_starts(&md_ctx, digest, 32) != 0) {
            mbedtls_fprintf(stderr,
                            "mbedtls_md_hmac_starts() returned error\n");
            goto exit;
        }
        ...
        for (offset = 0;
            ...
            ilen =
                ...
                mbedtls_cipher_get_block_size(&cipher_ctx) : (unsigned int)(
                    filesize - offset);
            ...
            if (mbedtls_md_hmac_update(&md_ctx, output, olen) != 0) {
                mbedtls_fprintf(stderr,
                                "mbedtls_md_hmac_update() returned error\n");
                goto exit;
            }
        ...
        if (mbedtls_md_hmac_finish(&md_ctx, digest) != 0) {
            mbedtls_fprintf(stderr,
                            "mbedtls_md_hmac_finish() returned error\n");
            goto exit;
        }

        if (fwrite(digest, 1, mbedtls_md_get_size(md_info),
                   fout) != mbedtls_md_get_size(md_info)) {
            mbedtls_fprintf(stderr,
    ...
    if (mode == MODE_DECRYPT) {
        ...
        if (mbedtls_cipher_info_get_mode(cipher_info) != MBEDTLS_MODE_GCM &&
            ...
            mbedtls_fprintf(stderr,
                            "File content not a multiple of the block size (%u).\n",
                            mbedtls_cipher_get_block_size(&cipher_ctx));
            goto exit;
        }
        ...
        if (fread(buffer, 1, mbedtls_md_get_size(md_info),
                  fin) != mbedtls_md_get_size(md_info)) {
            mbedtls_fprintf(stderr,
