...
usage:
    ...
    if (opt.mode == MODE_PRIVATE) {
        ...
        if (mbedtls_pk_get_type(&key) == MBEDTLS_PK_ECKEY) {
            ...
            mbedtls_mpi_write_file("Q(X): ",
                                   &ecp->MBEDTLS_PRIVATE(Q).MBEDTLS_PRIVATE(X),
                                   16,
                                   NULL);
            mbedtls_mpi_write_file("Q(Y): ",
                                   &ecp->MBEDTLS_PRIVATE(Q).MBEDTLS_PRIVATE(Y),
                                   16,
                                   NULL);
            mbedtls_mpi_write_file("Q(Z): ",
                                   &ecp->MBEDTLS_PRIVATE(Q).MBEDTLS_PRIVATE(Z),
                                   16,
                                   NULL);
