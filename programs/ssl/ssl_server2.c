    ...
    *end = ',';
    ...
    while (p <= end) {
        ...
        if ((new->cert = mbedtls_calloc(1, sizeof(mbedtls_x509_crt))) == NULL ||
            (new->key =
                 mbedtls_calloc(1, sizeof(mbedtls_pk_context))) == NULL) {
            goto error;
        }
        ...
        if (mbedtls_x509_crt_parse_file(new->cert, crt_file) != 0 ||
            mbedtls_pk_parse_keyfile(new->key, key_file, "", rng_get,
                                     &rng) != 0) {
            goto error;
        }
        ...
        if (strcmp(crl_file, "-") != 0) {
            if ((new->crl =
                     mbedtls_calloc(1, sizeof(mbedtls_x509_crl))) == NULL) {
                goto error;
            }
...
static int ssl_async_resume(mbedtls_ssl_context *ssl,
                            ...
                            size_t output_size)
{
    ssl_async_operation_context_t *ctx = mbedtls_ssl_get_async_operation_data(
        ssl);
    ssl_async_key_context_t *config_data =
        mbedtls_ssl_conf_get_async_config_data(ssl->conf);
...
int main(int argc, char *argv[])
{
    ...
    unsigned char dtls_srtp_key_material[
...
usage:
        ...
        *q++ = '\0';
        ...
        defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED)
            ...
            const mbedtls_cipher_info_t *ci =
                mbedtls_cipher_info_from_string(q);
...
#endif
    {
        ...
        for (i = 0; mbedtls_test_cas[i] != NULL; i++) {
            ret = mbedtls_x509_crt_parse(&cacert,
                                         (const unsigned char *)mbedtls_test_cas[
                                             i],
    ...
    if (strlen(opt.crt_file2) && strcmp(opt.crt_file2, "none") != 0) {
        key_cert_init2++;
        if ((ret =
                 mbedtls_x509_crt_parse_file(&srvcert2, opt.crt_file2)) != 0) {
            mbedtls_printf(
...
handshake:
    ...
    mbedtls_printf(
        ...
        (unsigned long)current_heap_memory,
        (unsigned long)peak_heap_memory);
#endif  /* MBEDTLS_MEMORY_DEBUG */
        ...
        if (opt.serialize == 2) {
            ...
            if (opt.nbio == 2) {
                ...
                mbedtls_ssl_set_bio(&ssl,
                                    ...
                                    mbedtls_net_recv,
                                    opt.nbio ==
                                    0 ? mbedtls_net_recv_timeout : NULL);
            }
