...
component_test_full_block_cipher_psa_dispatch_static_keystore () {
    ...
    # PSA key for AES.
    scripts/config.py unset MBEDTLS_PSA_KEY_SLOT_DYNAMIC
...
component_test_full_static_keystore () {
    msg "build: full config - MBEDTLS_PSA_KEY_SLOT_DYNAMIC"
    scripts/config.py full
    scripts/config.py unset MBEDTLS_PSA_KEY_SLOT_DYNAMIC
    make CC=clang CFLAGS="$ASAN_CFLAGS -Os" LDFLAGS="$ASAN_CFLAGS"

    msg "test: full config - MBEDTLS_PSA_KEY_SLOT_DYNAMIC"
    make test
}
