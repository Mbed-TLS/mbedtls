...
import jinja2
from asn1crypto import pem, x509  # type: ignore #pylint: disable=import-error


JINJA2_ENV = None
def get_jinja2_env():
    global JINJA2_ENV  #pylint: disable=global-statement
...
def generate_ca(args):
    write_target_file(args.template,
                      args.output or '{}.opensslconf'.format(args.ca_name),
                      ca_name=args.ca_name,
                      database=args.database or '{}.db'.format(args.ca_name))
...
def get_openssl_subject(cert):
    ...
    for tag, value in cert.subject.native.items():
        ret.append('{}={}'.format(openssl_trans_map[tag], value))
    return '/'.join(ret)
...
def get_cert_serial_subj(cert_file):
    with open(cert_file, 'rb') as f:
        for type_name, headers, der_bytes in pem.unarmor(f.read(), multiple=True):
            ...
            serial_number = '{:02x}'.format(cert.serial_number)
    ...
    with open(args.database, 'w') as f:
        for cert_file in args.certificates:
            for expired_date, serial, subject in get_cert_serial_subj(cert_file):
                ...
                f.write('R\t{}\t{}\t{}\tunkown\t{}\n'.format(
                    expired_date, revoke_date, serial, subject))
...
def main():
    ...
    ca_parser.add_argument('--template', type=str,
                           nargs='?', default='ca.opensslconf.jinja2')
    ...
    db_parser.add_argument('--database', type=str,
                           nargs='?', default='test-ca.db')
