...
if [ $INTERNAL ]
then
    HEADERS=$( ls include/mbedtls/*_internal.h crypto/include/mbedtls/*_internal.h library/*.h | egrep -v 'compat-1\.3\.h|bn_mul' )
else
    HEADERS=$( ls include/mbedtls/*.h crypto/include/mbedtls/*.h library/*.h | egrep -v 'compat-1\.3\.h|bn_mul' )
fi
