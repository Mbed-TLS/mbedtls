...
static int check_key_attributes_sanity(mbedtls_svc_key_id_t key)
{
    ...
    TEST_ASSERT(PSA_BLOCK_CIPHER_BLOCK_LENGTH(
                    type) <= PSA_BLOCK_CIPHER_BLOCK_MAX_SIZE);
                             ...
                             psa_algorithm_t alg)
{
    ...
    if (alg & PSA_ALG_AEAD_AT_LEAST_THIS_LENGTH_FLAG) {
        alg =
            PSA_ALG_AEAD_WITH_SHORTENED_TAG(alg,
    ...
    mbedtls_svc_key_id_t key)
{
    ...
    public_key_length = PSA_EXPORT_PUBLIC_KEY_OUTPUT_SIZE(public_key_type,
                                                          key_bits);
    ...
    const uint8_t *exported, size_t exported_length)
{
    ...
    if (type == PSA_KEY_TYPE_RSA_KEY_PAIR) {
        ...
        if (!mbedtls_test_asn1_skip_integer(&p, end, bits / 2, bits / 2 + 1,
                                            1)) {
            goto exit;
        }
        if (!mbedtls_test_asn1_skip_integer(&p, end, bits / 2, bits / 2 + 1,
