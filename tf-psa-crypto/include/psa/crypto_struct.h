...
struct psa_generate_key_iop_s {
     ...
     */
    unsigned int MBEDTLS_PRIVATE(id);
    psa_driver_generate_key_context_t MBEDTLS_PRIVATE(ctx);
...
#else
#define PSA_GENERATE_KEY_IOP_INIT { 0, { 0 }, 0, 0 }
#endif
...
struct psa_export_public_key_iop_s {
     ...
     */
    unsigned int MBEDTLS_PRIVATE(id);
    psa_driver_export_public_key_context_t MBEDTLS_PRIVATE(ctx);
    unsigned int MBEDTLS_PRIVATE(error_occurred) : 1;
#endif
};
